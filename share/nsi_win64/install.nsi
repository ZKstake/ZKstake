...
; Settings 
Name "ZKstake (ZKstake-Qt)"
OutFile "zkstake-qt-install.exe"
; RequestExecutionLevel admin
InstallDir "$PROGRAMFILES64\ZKstake"
Icon "zkstake.ico"
CRCCheck on
...
Section "Install ZKstake-Qt (GUI)" Section1
  SetOutPath $INSTDIR
  File "content\zkstake-qt-win.exe"
  CreateShortCut "$DESKTOP\ZKstake-Qt.lnk" "$INSTDIR\zkstake-qt-win.exe"
SectionEnd
...
Section "Create zkstake.conf" Section2
  SetOverwrite on
  CreateDirectory "$APPDATA\ZKstake"  
  SetOutPath $APPDATA\ZKstake
  File "content\zkstake.conf"
SectionEnd