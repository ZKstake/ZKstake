...
//extern bool zkstake_dailysnapshot(int32_t height);  //todo remove
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "zkstake.conf"));
...
#ifndef _WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "zkstaked.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 17838, 17771));
    ...
    strUsage += HelpMessageGroup(_("ZKstake Asset Chain options:"));
...
void CleanupBlockRevFiles()
{
    ...
    path zkstakestate = GetDataDir() / ZKSTAKE_STATE_FILENAME;
    remove(zkstakestate);
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ZKstake is shutting down."));
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ZKstake is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ZKstake is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                if (fReindex) {
                    boost::filesystem::remove(GetDataDir() / ZKSTAKE_STATE_FILENAME);
                ...
                if ( ASSETCHAINS_CC != 0 && ZKSTAKE_SNAPSHOT_INTERVAL != 0 && chainActive.Height() >= ZKSTAKE_SNAPSHOT_INTERVAL )
                {
                    if ( !zkstake_dailysnapshot(chainActive.Height()) )
                    {
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of ZKstake") << "\n";
