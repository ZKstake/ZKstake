...
#include "miner.h" // for zkstake_sendmessage
...
int32_t games_event(uint32_t timestamp,uint256 gametxid,int32_t eventid,std::vector<uint8_t> payload)
{
    ...
    if ( games_eventsign(timestamp,sig,payload,mypk) == 0 )
    {
        ...
        zkstake_sendmessage(4,8,"events",vopret);
        return(0);
    }
...
void zkstake_netevent(std::vector<uint8_t> message)
{
        ...
        else if ( lagerr == 0 )
        {
            if ( games_payloadrecv(pk,timestamp,payload) == 0 ) // first time this is seen
            {
                if ( (rand() % 10) == 0 )
                {
                    ...
                    zkstake_sendmessage(2,2,"events",message);
                }
            }
        }
...
int32_t games_playersalive(int32_t &openslots,int32_t &numplayers,uint256 gametxid,int32_t maxplayers,int32_t gameht,CTransaction gametx)
{
    ...
    if ( zkstake_nextheight() <= gameht+GAMES_MAXKEYSTROKESGAP )
        registration_open = 1;
    for (i=0; i<maxplayers; i++)
    {
    ...
    if ( myGetTransaction(gametxid,tx,hashBlock) != 0 && (pindex= zkstake_blockindex(hashBlock)) != 0 )
    {
        ...
        if ( zkstake_nextheight() > ht+delay )
        {
            if ( (pindex= zkstake_chainactive(ht+delay)) != 0 )
            {
                ...
                else sprintf(cmd,"./zkstake-cli -ac_name=%s cclib register %d \"[%%22%s%%22]\"",chainName.symbol().c_str(),EVAL_GAMES,gametxid.ToString().c_str());
    ...
    if ( params != 0 && (n= cJSON_GetArraySize(params)) > 0 )
    {
        if ( n > 0 )
        {
            ...
            if ( (err= games_isvalidgame(cp,gameheight,tx,buyin,maxplayers,gametxid,1)) == 0 )
            {
                ...
                if ( zkstake_nextheight() > gameheight + GAMES_MAXKEYSTROKESGAP )
