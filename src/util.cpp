...
/****
 * @brief get the OS-specific default zkstaked data directory
 * @note Windows: be "C:\Users\[username]\AppData\Roaming\ZKstake"
 * @note Mac: ~/Library/Application Support/ZKstake
 * @note Unix: ~/.zkstake
 * @returns the default path to the ZKstake data directory
 */
...
{    
    ...
    // Unix
    if (chainName.isKMD())
        return pathRet / ".zkstake";
    else 
        return pathRet / ".zkstake" / chainName.symbol();
#endif
}
 ...
 * @returns the full OS-specific data directory including ZKstake (i.e. "~/.zkstake")
 */
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override zkstake.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "zkstaked.pid"));
...
std::string LicenseInfo()
{
    ...
    const std::string URL_SOURCE_CODE = "<https://github.com/zkstakecoin/ZKstake>";
    ...
    return "\n" +
		   ...
		   FormatParagraph(strprintf(_("Copyright (C) 2023-%i ZKstake developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure ZKstake Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("ZKstake Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The ZKstake Core developers";
    }
