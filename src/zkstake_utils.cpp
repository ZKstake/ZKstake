...
int32_t zkstake_opreturnscript(uint8_t *script,uint8_t type,uint8_t *opret,int32_t opretlen)
{
    int32_t offset = 0;
    script[offset++] = 0x6a;
    opretlen++;
...
uint64_t zkstake_block_prg(uint32_t nHeight)
{
...
int64_t zkstake_block_unlocktime(uint32_t nHeight)
{
    ...
    {
        unlocktime = zkstake_block_prg(nHeight) % (ASSETCHAINS_TIMEUNLOCKTO - ASSETCHAINS_TIMEUNLOCKFROM);
 ...
 * @return the RPC port
 */
uint16_t _zkstake_userpass(char *username,char *password,FILE *fp)
{
    uint16_t port = 0;
...
void zkstake_statefname(char *fname, const char *symbol, const char *str)
{
    int32_t n,len;
    ...
    if ( (n= (int32_t)chainName.symbol().size()) != 0 )
    {
        ...
        if ( !mapArgs.count("-datadir") && strcmp(chainName.symbol().c_str(),&fname[len - n]) == 0 )
            fname[len - n] = 0;
        else if(mapArgs.count("-datadir")) LogPrintf("DEBUG - zkstake_utils:1363: custom datadir\n");
        else
        {
...
void zkstake_configfile(const char *symbol,uint16_t rpcport)
{
    ...
    if ( symbol != 0 && rpcport != 0 )
    {
        ...
        {
            _zkstake_userpass(myusername,mypassword,fp);
    ...
    if ( (fp= fopen(fname,"rb")) != 0 )
    {
        if ( (kmdport= _zkstake_userpass(username,password,fp)) != 0 )
            ZKSB_PORT = kmdport;
        sprintf(ZKSBUSERPASS,"%s:%s",username,password);
        fclose(fp);
    }
}

uint16_t zkstake_userpass(char *userpass,const char *symbol)
{
    FILE *fp; uint16_t port = 0; char fname[512],username[512],password[512],confname[ZKSTAKE_ASSETCHAIN_MAXLEN];
    userpass[0] = 0;
    if ( strcmp("KMD",symbol) == 0 )
    {
#ifdef __APPLE__
        sprintf(confname,"ZKstake.conf");
#else
        sprintf(confname,"zkstake.conf");
#endif
    }
        ...
        zkstake_statefname(fname,symbol,confname);
    } else sprintf(fname,"%s",GetDataDir().string().c_str());
    
    if ( (fp= fopen(fname,"rb")) != 0 )
    {
        port = _zkstake_userpass(username,password,fp);
 ...
 */
uint32_t zkstake_assetmagic(const char *symbol,uint64_t supply,uint8_t *extraptr,int32_t extralen)
{
    uint8_t buf[512]; 
    uint32_t crc0=0; 
    int32_t len = 0; 
 ...
 */
uint16_t zkstake_assetport(uint32_t magic,int32_t extralen)
{
    if ( magic == 0x8de4eef9 )
        return 7770;
    else if ( extralen == 0 )
 ...
 */
uint16_t zkstake_port(const char *symbol,uint64_t supply,uint32_t *magicp,uint8_t *extraptr,int32_t extralen)
{
    if ( symbol == 0 || symbol[0] == 0 || strcmp("KMD",symbol) == 0 )
    {
        *magicp = 0x8de4eef9;
    ...
    *magicp = zkstake_assetmagic(symbol,supply,extraptr,extralen);
    return zkstake_assetport(*magicp,extralen);
}


int32_t zkstake_whoami(char *pubkeystr,int32_t height,uint32_t timestamp)
{
    int32_t i,notaryid;
    for (i=0; i<33; i++)
        sprintf(&pubkeystr[i<<1],"%02x",NOTARY_PUBKEY33[i]);
    pubkeystr[66] = 0;
    zkstake_chosennotary(&notaryid,height,NOTARY_PUBKEY33,timestamp);
    return(notaryid);
}
...
uint64_t zkstake_current_supply(uint32_t nHeight)
{
    ...
    if ( ZKSTAKE_BIT63SET(cur_money) != 0 )
        return(ZKSTAKE_MAXNVALUE);
    if ( ASSETCHAINS_COMMISSION != 0 )
    {
        uint64_t newval = (cur_money + (cur_money/COIN * ASSETCHAINS_COMMISSION));
        if ( ZKSTAKE_BIT63SET(newval) != 0 )
            return(ZKSTAKE_MAXNVALUE);
...
uint64_t zkstake_max_money()
{
    return zkstake_current_supply(10000000);
}

uint64_t zkstake_ac_block_subsidy(int nHeight)
{
...
int8_t equihash_params_possible(uint64_t n, uint64_t k)
{
    ...
    *  EhOptimisedSolve
    *  EhIsValidSolution
    * Alternatively change ASSETCHAINS_N and ASSETCHAINS_K in zkstake_nk.h for fast testing.
    */
    if ( k == 9 && (n == 200 || n == 210) )
        return(0);
 ...
 * @note modifies ASSETCHAINS_P2PPORT, ASSETCHAINS_RPCPORT, ZKSBUSERPASS, BTCUSERPASS, DESTPORT
 * @note IS_ZKSTAKE_NOTARY should already be set
...
{
    ASSETCHAINS_P2PPORT = 48136; // default port for P2P
    ASSETCHAINS_RPCPORT = 17838; // default port for RPC
#ifdef __APPLE__
    std::string filename = "ZKstake.conf";
#else
    std::string filename = "zkstake.conf";
#endif
    ...
    if (!userpass.empty())
        strncpy(ZKSBUSERPASS, userpass.c_str(), 8705);
...
void zkstake_args(char *argv0)
{
    ...
    IS_ZKSTAKE_NOTARY = ntz_dest_path == "" ? 0 : 1;


    STAKED_NOTARY_ID = GetArg("-stakednotary", -1);
    ZKSTAKE_NSPV = GetArg("-nSPV",0);
    memset(disablebits,0,sizeof(disablebits)); // everything enabled
    if ( GetBoolArg("-gen", false) != 0 )
    {
        ZKSTAKE_MININGTHREADS = GetArg("-genproclimit",-1);
    }
    ...
    IS_ZKSTAKE_DEALERNODE = GetArg("-dealer",0);
    IS_ZKSTAKE_TESTNODE = GetArg("-testnode",0);
    ...
    if ( strlen(NOTARY_PUBKEY.c_str()) == 66 )
    {
        ...
        if ( !IS_ZKSTAKE_NOTARY )
        {
            ...
            for (uint16_t i=0; i<64; i++)
            {
                ...
                {
                    IS_ZKSTAKE_NOTARY = true;
                    ZKSTAKE_MININGTHREADS = 1;
                    mapArgs ["-genproclimit"] = itostr(ZKSTAKE_MININGTHREADS);
                    STAKED_NOTARY_ID = -1;
    ...
    if ( STAKED_NOTARY_ID != -1 && IS_ZKSTAKE_NOTARY == true ) {
        LogPrintf( "Cannot be STAKED and KMD notary at the same time!\n");
        StartShutdown();
    }
    SoftSetArg("-ac_name", std::string("ZKSB"));
    SoftSetArg("-ac_supply", std::string("0"));
    SoftSetArg("-ac_halving", std::string("100000"));
    SoftSetArg("-ac_blocktime", std::string("120"));
    SoftSetArg("-ac_reward", std::string("100000000"));
    SoftSetArg("-ac_decay", std::string("75000000"));
    SoftSetArg("-ac_staked", std::string("70"));
    SoftSetArg("-ac_sapling", std::string("1"));
    SoftSetArg("-ac_adaptivepow", std::string("1"));
    SoftSetArg("-ac_perc", std::string("5000000"));
    SoftSetArg("-ac_founders", std::string("100"));
    SoftSetArg("-ac_pubkey", std::string("029c10d47469debc48f45345c13a6277a010d50a1ec5b065b9cc53356ffb2ca32a"));
    SoftSetArg("-ac_cbmaturity", std::string("1"));
    SoftSetArg("-ac_cc", std::string("0"));
    SoftSetArg("-addnode", std::string("167.172.130.118"));
    SoftSetArg("-addnode", std::string("157.230.90.81"));
    SoftSetArg("-addnode", std::string("70.177.179.193"));
    ...
    if ( (ZKSTAKE_REWIND= GetArg("-rewind",0)) != 0 )
    {
        LogPrintf("ZKSTAKE_REWIND %d\n",ZKSTAKE_REWIND);
    }
    ZKSTAKE_EARLYTXID = Parseuint256(GetArg("-earlytxid","0").c_str());    
    ...
    if ( !chainName.isKMD() )
    {
                ...
                || ASSETCHAINS_ADAPTIVEPOW != 0 )
        {
            ...
            if ( ZKSTAKE_SNAPSHOT_INTERVAL != 0 )
            {
                extralen += iguana_rwnum(1,&extraptr[extralen],sizeof(ZKSTAKE_SNAPSHOT_INTERVAL),(void *)&ZKSTAKE_SNAPSHOT_INTERVAL);
            }
        ...
        MAX_MONEY = zkstake_max_money();
        int32_t baseid;
        if ( (baseid = zkstake_baseid(chainName.symbol().c_str())) >= 0 && baseid < 32 )
        {
            LogPrintf("baseid.%d MAX_MONEY.%s %.8f\n",baseid,chainName.symbol().c_str(),(double)MAX_MONEY/SATOSHIDEN);
        }

        if ( ASSETCHAINS_CC >= ZKSTAKE_FIRSTFUNGIBLEID && MAX_MONEY < 1000000LL*SATOSHIDEN )
            MAX_MONEY = 1000000LL*SATOSHIDEN;
        if ( ZKSTAKE_BIT63SET(MAX_MONEY) != 0 )
            MAX_MONEY = ZKSTAKE_MAXNVALUE;
        LogPrintf("MAX_MONEY %llu %.8f\n",(long long)MAX_MONEY,(double)MAX_MONEY/SATOSHIDEN);
        uint16_t tmpport = zkstake_port(chainName.symbol().c_str(),ASSETCHAINS_SUPPLY,&ASSETCHAINS_MAGIC,extraptr,extralen);
        if ( GetArg("-port",0) != 0 )
        {
        ...
        if ( !chainName.isKMD() )
        {
            ...
            uint16_t port;
            if ( (port= zkstake_userpass(ASSETCHAINS_USERPASS,chainName.symbol().c_str())) != 0 )
                ASSETCHAINS_RPCPORT = port;
            else zkstake_configfile(chainName.symbol().c_str(),ASSETCHAINS_P2PPORT + 1);
    ...
    int32_t dpowconfs = ZKSTAKE_DPOWCONFS;
    if ( !chainName.isKMD() )
    {
    ...
    ZKSTAKE_DPOWCONFS = GetArg("-dpowconfs",dpowconfs);
    if ( chainName.isKMD() 
 ...
 */
void zkstake_nameset(char *symbol,char *dest,const char *source)
{
...
/****
 * @brief get the right zkstake_state
 * @param[in] base what to search for (nullptr == "KMD")
 * @returns the correct zkstake_state object
 */
zkstake_state *zkstake_stateptrget(char *base)
{
    // "KMD" case
    if ( base == 0 || base[0] == 0 || strcmp(base,(char *)"KMD") == 0 )
        return &ZKSTAKE_STATES[1];

    // evidently this asset chain
    return &ZKSTAKE_STATES[0];
}
 ...
 * @returns the zkstake_state object for symbol
 */
zkstake_state *zkstake_stateptr(char *symbol,char *dest)
{
    int32_t baseid;
    zkstake_nameset(symbol,dest,chainName.symbol().c_str());
    return(zkstake_stateptrget(symbol));
}
...
bool zkstake_is_vSolutionsFixActive()
{
    return GetLatestTimestamp(zkstake_currentheight()) > nS5Timestamp;
}
