...
// Copyright (c) 2015-2020 The ZKstake Platform developers
...
const string strMessageMagic = "ZKstake Signed Message:\n";
...
UniValue zkstake_snapshot(int top)
{
    LOCK(cs_main);
    int64_t total = -1;
...
bool zkstake_snapshot2(std::map <std::string, CAmount> &addressAmounts)
{
...
bool zkstake_dailysnapshot(int32_t height)
{
    int reorglimit = 100; 
    ...
    // Under heavy reorg attack, its possible `zkstake_notarized_height` can return a height that can't be found on chain sync.
    ...
    if ( (extraoffset= height % ZKSTAKE_SNAPSHOT_INTERVAL) != 0 )
    {
    ...
    else 
    {
        ...
        notarized_height = zkstake_notarized_height(&prevMoMheight,&notarized_hash,&notarized_desttxid);
    ...
    if ( !zkstake_snapshot2(addressAmounts) )
        return false;
    ...
    for (int32_t n = height; n > undo_height; n--) 
    {
        ...
        if ( (pindex= zkstake_chainactive(n)) == 0 || zkstake_blockload(block, pindex) != 0 ) 
            return false;
...
bool IsFinalTx(const CTransaction &tx, int nBlockHeight, int64_t nBlockTime)
{
    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin)
    {
        if ( !zkstake_hardfork_active(nBlockTime) && txin.nSequence == 0xfffffffe &&
        ...
        else if ( zkstake_hardfork_active(nBlockTime) && txin.nSequence == 0xfffffffe &&
            (
                ((int64_t)tx.nLockTime >= LOCKTIME_THRESHOLD && (int64_t)tx.nLockTime <= nBlockTime) ||
                ((int64_t)tx.nLockTime <  LOCKTIME_THRESHOLD && (int64_t)tx.nLockTime <= nBlockHeight))
            )
...
bool ContextualCheckCoinbaseTransaction(int32_t slowflag,const CBlock *block,CBlockIndex * const previndex,const CTransaction& tx, const int nHeight,int32_t validateprices)
{
    ...
    if (((uint64_t)(tx.GetValueOut()) >= ASSETCHAINS_TIMELOCKGTE) || (zkstake_ac_block_subsidy(nHeight) >= ASSETCHAINS_TIMELOCKGTE))
    {
            ...
            if (tx.vout[1].scriptPubKey.GetOp2(it, op, &opretData))
            {
                if (opretData.size() > 0 && opretData.data()[0] == OPRETTYPE_TIMELOCK)
                {
                    ...
                    if (CScriptID(opretScript) == scriptHash &&
                        ...
                        zkstake_block_unlocktime(nHeight) == unlocktime)
                    {
                        return(true);
                    }
                }
                      ...
                      libzcash::ProofVerifier& verifier,int32_t txIndex, int32_t numTxs)
{
    if ( chainName.isKMD() )
    {
        ...
        if ( *(int32_t *)&array[0] == 0 )
            numbanned = zkstake_bannedset(&indallvouts,array,(int32_t)(sizeof(array)/sizeof(*array)));

        for (size_t j=0; j<tx.vin.size(); j++) // for every tx.vin
        {
            for (int32_t k=0; k<numbanned; k++) // go through the array of banned txids
            {
                if ( tx.vin[j].prevout.hash == array[k] && zkstake_checkvout(tx.vin[j].prevout.n,k,indallvouts) != 0 ) //(tx.vin[j].prevout.n == 1 || k >= indallvouts) )
                {
    ...
    uint256 merkleroot;
    if ( ASSETCHAINS_STAKED != 0 && zkstake_newStakerActive(0, tiptime) != 0 && tx.vout.size() == 2 && DecodeStakingOpRet(tx.vout[1].scriptPubKey, merkleroot) != 0 )
    {
...
bool CheckTransactionWithoutProofVerification(uint32_t tiptime,const CTransaction& tx, CValidationState &state)
{
    ...
    int32_t invalid_private_taddr=0,z_z=0,z_t=0,t_z=0,acpublic = zkstake_acpublic(tiptime), current_season = getacseason(tiptime);
    /**
     * Previously:
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
        ...
        if ( ASSETCHAINS_PRIVATE != 0 )
        {
            ...
            if (iscoinbase == 0 && txout.nValue > 0)
            {
                ...
                if ( zkstake_isnotaryvout(destaddr,tiptime) == 0 )
                {
    ...
    BOOST_FOREACH(const JSDescription& joinsplit, tx.vjoinsplit)
    {
        ...
        if ( tiptime >= ZKSTAKE_SAPLING_DEADLINE )
        {
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState &state, const CTransaction &tx, bool fLimitFree,bool* pfMissingInputs, bool fRejectAbsurdFee, int dosLevel)
{
    ...
    if ( chainName.isKMD() && chainActive.Tip() != nullptr
            && !zkstake_validate_interest(tx,chainActive.Tip()->nHeight+1, chainActive.Tip()->GetMedianTimePast() + 777))
    {
        return state.DoS(0, error("%s: zkstake_validate_interest failed txid.%s", __func__, tx.GetHash().ToString()), REJECT_INVALID, "zkstake-interest-invalid");
    }
...

    {
        ...
        if ( nValueOut > 777777*COIN && ZKSTAKE_VALUETOOBIG(nValueOut - 777777*COIN) != 0 ) // some room for blockreward and txfees
        ...
        if ( ZKSTAKE_CONNECTING <= 0 && chainActive.Tip() != 0 )
        {
            flag = 1;
            ZKSTAKE_CONNECTING = (1<<30) + (int32_t)chainActive.Tip()->nHeight + 1;
        }

        if (!ContextualCheckInputs(tx, state, view, true, MANDATORY_SCRIPT_VERIFY_FLAGS, true, txdata, Params().GetConsensus(), consensusBranchId))
        {
            if ( flag != 0 )
                ZKSTAKE_CONNECTING = -1;
...
bool ReadBlockFromDisk(int32_t height,CBlock& block, const CDiskBlockPos& pos,bool checkPOW)
{
    ...
    if ( 0 && checkPOW != 0 )
    {
        zkstake_block2pubkey33(pubkey33,(CBlock *)&block);
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if ( chainName.isKMD() )
    {
        ...
        else if ( nHeight < ZKSTAKE_ENDOFERA )
            return(3 * COIN);
        else if ( nHeight < 2*ZKSTAKE_ENDOFERA )
            return(2 * COIN);
        else return(COIN);
    }
    else
    {
        return(zkstake_ac_block_subsidy(nHeight));
    }
    /*
     // Mining slow start
    ...
    bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, const Consensus::Params& consensusParams)
    {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                ...
                {
                    uint64_t unlockTime = zkstake_block_unlocktime(coins->nHeight);
...
#ifdef ZKSTAKE_ENABLE_INTEREST
            if ( chainName.isKMD() && nSpendHeight > 60000 )
            {
                if ( coins->vout[prevout.n].nValue >= 10*COIN )
                {
                    int64_t interest; int32_t txheight; uint32_t locktime;
                    if ( (interest= zkstake_accrued_interest(&txheight,&locktime,prevout.hash,prevout.n,0,coins->vout[prevout.n].nValue,(int32_t)nSpendHeight-1)) != 0 )
                    {
                        nValueIn += interest;
                    }
                }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    bool fClean = true;
    //zkstake_disconnect(pindex,block); does nothing?
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck,bool fCheckPOW)
{
    ...
    if ( ZKSTAKE_NSPV_SUPERLITE )
        return(true);
    if ( ZKSTAKE_STOPAT != 0 && pindex->nHeight > ZKSTAKE_STOPAT )
        return(false);
    ...
    if ( ASSETCHAINS_NOTARY_PAY[0] != 0 && pindex->nHeight > 10 )
    {
        ...
        int32_t notarisationTx = zkstake_connectblock(true,pindex,*(CBlock *)&block);  
        ...
        if ( notarisationTx == 1 )
        {
            ...
            notarypaycheque = zkstake_checknotarypay((CBlock *)&block,(int32_t)pindex->nHeight);
    ...
    //if ( ZKSTAKE_TESTNET_EXPIRATION != 0 && pindex->nHeight > ZKSTAKE_TESTNET_EXPIRATION ) // "testnet"
    //    return(false);
    ...
    */
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if ( ZKSTAKE_VALUETOOBIG(valueout) != 0 )
        {
    ...
    if ( ASSETCHAINS_STAKED != 0 && fCheckPOW && zkstake_checkPOW(blockReward+stakeTxValue-notarypaycheque,1,(CBlock *)&block,pindex->nHeight) < 0 ) 
        return state.DoS(100, error("ConnectBlock: ac_staked chain failed slow zkstake_checkPOW"),REJECT_INVALID, "failed-slow_checkPOW");
    ...
    if ( chainName.isKMD() && pindex->nHeight >= ZKSTAKE_NOTARIES_HEIGHT2)
        blockReward -= sum;

    if ( ASSETCHAINS_COMMISSION != 0 || ASSETCHAINS_FOUNDERS_REWARD != 0 ) //ASSETCHAINS_OVERRIDE_PUBKEY33[0] != 0 &&
    {
        uint64_t checktoshis;
        if ( (checktoshis= zkstake_commission((CBlock *)&block,(int32_t)pindex->nHeight)) != 0 )
        {
    ...
    if ( block.vtx[0].GetValueOut() > blockReward+ZKSTAKE_EXTRASATOSHI )
    {
        if ( !chainName.isKMD() || pindex->nHeight >= ZKSTAKE_NOTARIES_HEIGHT1 || block.vtx[0].vout[0].nValue > blockReward )
        {
    ...
    //FlushStateToDisk();
    zkstake_connectblock(false,pindex,*(CBlock *)&block);  // dPoW state update.
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    {
        int32_t longestchain = zkstake_longestchain();
...
bool static DisconnectTip(CValidationState &state, bool fBare = false) {
    ...
    //if ( ASSETCHAINS_SYMBOL[0] != 0 || pindexDelete->nHeight > 1400000 )
    {
        ...
        notarizedht = zkstake_notarized_height(&prevMoMheight,&notarizedhash,&txid);
    ...
    if (!fBare) {
        ...
        for (int i = 0; i < block.vtx.size(); i++)
        {
            ...
            if (tx.IsCoinBase() || (i == block.vtx.size()-1 && zkstake_newStakerActive(0, pindexDelete->nTime) == 0 && zkstake_isPoS((CBlock *)&block,pindexDelete->nHeight,0) != 0) || !AcceptToMemoryPool(mempool, stateDummy, tx, false, NULL))
            {
    ...
    {
        CTransaction &tx = block.vtx[i];
        //if ((i == (block.vtx.size() - 1)) && ((ASSETCHAINS_STAKED != 0 && (zkstake_isPoS((CBlock *)&block) != 0))))
        if ( zkstake_newStakerActive(0, pindexDelete->nTime) == 0 && i == block.vtx.size()-1 && zkstake_isPoS((CBlock *)&block,pindexDelete->nHeight,0) != 0 )
        {
#ifdef ENABLE_WALLET
             // new staking tx cannot be accepted to mempool and expires in 1 block, so no need for this! :D
             if ( !GetBoolArg("-disablewallet", false) && ZKSTAKE_NSPV_FULLNODE )
...
int32_t zkstake_activate_sapling(CBlockIndex *pindex)
{
    uint32_t blocktime,prevtime; CBlockIndex *prev; int32_t i,transition=0,height,prevht;
    int32_t activation = 0;
    if ( pindex == 0 )
    {
        LogPrintf("zkstake_activate_sapling null pindex\n");
        return(0);
    }
    height = pindex->nHeight;
    blocktime = (uint32_t)pindex->nTime;
    //LogPrintf("zkstake_activate_sapling.%d starting blocktime %u cmp.%d\n",height,blocktime,blocktime > ZKSTAKE_SAPLING_ACTIVATION);
    ...
    height = pindex->nHeight;
    blocktime = (uint32_t)pindex->nTime;
    //LogPrintf("starting blocktime %u cmp.%d\n",blocktime,blocktime > ZKSTAKE_SAPLING_ACTIVATION);
    if ( blocktime > ZKSTAKE_SAPLING_ACTIVATION ) // find the earliest transition
    {
        while ( (prev= pindex->pprev) != 0 )
        {
            prevht = prev->nHeight;
            prevtime = (uint32_t)prev->nTime;
            //LogPrintf("(%d, %u).%d -> (%d, %u).%d\n",prevht,prevtime,prevtime > ZKSTAKE_SAPLING_ACTIVATION,height,blocktime,blocktime > ZKSTAKE_SAPLING_ACTIVATION);
            if ( prevht+1 != height )
            {
                LogPrintf("zkstake_activate_sapling: unexpected non-contiguous ht %d vs %d\n",prevht,height);
                return(0);
            }
            if ( prevtime <= ZKSTAKE_SAPLING_ACTIVATION && blocktime > ZKSTAKE_SAPLING_ACTIVATION )
            {
                activation = height + 60;
                LogPrintf("%s transition at %d (%d, %u) -> (%d, %u)\n",chainName.symbol().c_str(),height,prevht,prevtime,height,blocktime);
            }
            if ( prevtime < ZKSTAKE_SAPLING_ACTIVATION-3600*24 )
                break;
            pindex = prev;
            height = prevht;
            blocktime = prevtime;
    ...
    if ( activation != 0 )
    {
        zkstake_setactivation(activation);
...
bool static ConnectTip(CValidationState &state, CBlockIndex *pindexNew, CBlock *pblock) {
    ...
    ZKSTAKE_CONNECTING = (int32_t)pindexNew->nHeight;
    ...
    if ( ZKSTAKE_LONGESTCHAIN != 0 && (pindexNew->nHeight == ZKSTAKE_LONGESTCHAIN || pindexNew->nHeight == ZKSTAKE_LONGESTCHAIN+1) )
        ZKSTAKE_INSYNC = (int32_t)pindexNew->nHeight;
    else ZKSTAKE_INSYNC = 0;
    //LogPrintf("connect.%d insync.%d ASSETCHAINS_SAPLING.%d\n",(int32_t)pindexNew->nHeight,ZKSTAKE_INSYNC,ASSETCHAINS_SAPLING);
    /*if ( ZKSTAKE_INSYNC != 0 ) //ASSETCHAINS_SYMBOL[0] == 0 &&
        zkstake_broadcast(pblock,8);
    else if ( ASSETCHAINS_SYMBOL[0] != 0 )
        zkstake_broadcast(pblock,4);*/
    if ( ZKSTAKE_NSPV_FULLNODE )
    {
        if ( ASSETCHAINS_SAPLING <= 0 && pindexNew->nTime > ZKSTAKE_SAPLING_ACTIVATION - 24*3600 )
            zkstake_activate_sapling(pindexNew);
        if ( ASSETCHAINS_CC != 0 && ZKSTAKE_SNAPSHOT_INTERVAL != 0 && (pindexNew->nHeight % ZKSTAKE_SNAPSHOT_INTERVAL) == 0 && pindexNew->nHeight >= ZKSTAKE_SNAPSHOT_INTERVAL )
        {
            uint64_t start = time(NULL);
            if ( !zkstake_dailysnapshot(pindexNew->nHeight) )
            {
...
static bool ActivateBestChainStep(bool fSkipdpow, CValidationState &state, CBlockIndex *pindexMostWork, CBlock *pblock) {
    ...
    if ( ZKSTAKE_REWIND != 0 )
    {
        ...
        LogPrintf(">>>>>>>>>>> rewind start ht.%d -> ZKSTAKE_REWIND.%d\n",
                chainActive.Tip()->nHeight,ZKSTAKE_REWIND);
        while ( ZKSTAKE_REWIND > 0 && (tipindex= chainActive.Tip()) != 0 && tipindex->nHeight > ZKSTAKE_REWIND )
        {
            fBlocksDisconnected = true;
        ...
        LogPrintf("reached rewind.%d, best to do: ./zkstake-cli -ac_name=%s stop\n",ZKSTAKE_REWIND,chainName.symbol().c_str());
#ifdef WIN32
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    */
    ...
    {
        if ( it->second != 0 ) // vNodes.size() >= ZKSTAKE_LIMITED_NETWORKSIZE, change behavior to allow zkstake_ensure to work
        {
...
bool CheckBlockHeader(int32_t *futureblockp,int32_t height,CBlockIndex *pindex, const CBlockHeader& blockhdr, CValidationState& state, bool fCheckPOW)
{
    ...
    *futureblockp = 0;
    ...
    /*zkstake_index2pubkey33(pubkey33,pindex,height);
                ...
                bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if ( fCheckPOW )
    {
        ...
        if ( ASSETCHAINS_STAKED == 0 && zkstake_checkPOW(0,1,(CBlock *)&block,height) < 0 ) // checks Equihash
    ...
    {
        int32_t notaryid;
        int32_t special = zkstake_chosennotary(&notaryid,height,pubkey33,tiptime);
        if (notaryid > 0 || ( notaryid == 0 && height > nS5HardforkHeight ) ) {
            ...
            if (!zkstake_checkopret(pblockcopy, merkleroot)) {
                LogPrintf("failed or missing merkleroot expected.%s != merkleroot.%s\n", zkstake_makeopret(pblockcopy, false).ToString().c_str(), merkleroot.ToString().c_str());
    ...
    if ( ASSETCHAINS_CC != 0 ) // CC contracts might refer to transactions in the current block, from a CC spend within the same block and out of order
    {
        ...
        while ( 1 )
        {
            ...
            for (i=0; i<block.vtx.size(); i++)
            {
                ...
                if ( tx.IsCoinBase() || !tx.vjoinsplit.empty() || !tx.vShieldedSpend.empty() || (i == block.vtx.size()-1 && zkstake_isPoS((CBlock *)&block,height,0) != 0) )
                    continue;
                Tx = tx;
    ...
    if ( fCheckPOW && zkstake_check_deposit(height,block) < 0 )
    {
        //static uint32_t counter;
        //if ( counter++ < 100 && ASSETCHAINS_STAKED == 0 )
        //    LogPrintf("check deposit rejection\n");
        LogPrintf("CheckBlockHeader zkstake_check_deposit error");
        return(false);
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (fCheckpointsEnabled)
    {
        ...
        if ( nHeight != 0 )
        {
            ...
            if ( !zkstake_checkpoint(&notarized_height,nHeight,hash) )
            {
                CBlockIndex *heightblock = chainActive[nHeight];
                if ( heightblock != 0 && heightblock->GetBlockHash() == hash )
                    return true;
...
bool ContextualCheckBlock(int32_t slowflag,const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    const int32_t txheight = nHeight == 0 ? zkstake_block2height((CBlock *)&block) : nHeight;
    ...
    for (uint32_t i = 0; i < block.vtx.size(); i++) {
        ...
        // Interest validation
        if (!zkstake_validate_interest(tx, txheight, cmptime))
        {
            LogPrintf("validate intrest failed for txnum.%i tx.%s\n", i, tx.ToString());
            return state.DoS(0, error("%s: zkstake_validate_interest failed", __func__), REJECT_INVALID, "zkstake-interest-invalid");
        }
...
bool AcceptBlockHeader(int32_t *futureblockp,const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (miSelf != mapBlockIndex.end())
    {
        ...
        {
            if ( ASSETCHAINS_CC == 0 )//&& (ASSETCHAINS_PRIVATE == 0 || ZKSTAKE_INSYNC >= Params().GetConsensus().vUpgrades[Consensus::UPGRADE_SAPLING].nActivationHeight) )
        ...
        /*if ( pindex != 0 && hash == zkstake_requestedhash )
        {
            LogPrintf("AddToBlockIndex A zkstake_requestedhash %s\n",zkstake_requestedhash.ToString().c_str());
            memset(&zkstake_requestedhash,0,sizeof(zkstake_requestedhash));
            zkstake_requestedcount = 0;
        }*/
    ...
    /*if ( pindex != 0 && hash == zkstake_requestedhash )
    {
        LogPrintf("AddToBlockIndex zkstake_requestedhash %s\n",zkstake_requestedhash.ToString().c_str());
        memset(&zkstake_requestedhash,0,sizeof(zkstake_requestedhash));
        zkstake_requestedcount = 0;
    }*/
    return true;
...
CBlockIndex *zkstake_ensure(CBlock *pblock, uint256 hash)
{
...
CBlockIndex *oldzkstake_ensure(CBlock *pblock, uint256 hash)
{
    CBlockIndex *pindex=0,*previndex=0;
    if ( (pindex = zkstake_getblockindex(hash)) == 0 )
    {
        pindex = new CBlockIndex();
        if (!pindex)
            throw runtime_error("zkstake_ensure: new CBlockIndex failed");
    ...
    if ( miSelf == mapBlockIndex.end() )
    {
        LogPrintf("zkstake_ensure unexpected missing hash %s\n",hash.ToString().c_str());
        return(0);
    }
    if ( miSelf->second == 0 ) // create pindex so first Accept block doesnt fail
    {
        ...
        if ( pindex != 0 )
        {
            miSelf->second = pindex;
            LogPrintf("Block header %s is already known, but without pindex -> ensured %p\n",hash.ToString().c_str(),miSelf->second);
        } else LogPrintf("zkstake_ensure unexpected null pindex\n");
    }
    /*if ( hash != Params().GetConsensus().hashGenesisBlock )
        {
            miSelf = mapBlockIndex.find(pblock->hashPrevBlock);
            ...
            if ( (miSelf= mapBlockIndex.find(pblock->hashPrevBlock)) != mapBlockIndex.end() )
            {
                if ( miSelf->second == 0 ) // create pindex so first Accept block doesnt fail
                {
                    ...
                    if ( previndex != 0 )
                    {
                        ...
                        LogPrintf("autocreate previndex %s\n",pblock->hashPrevBlock.ToString().c_str());
                    } else LogPrintf("zkstake_ensure unexpected null previndex\n");
                }
            } else LogPrintf("zkstake_ensure unexpected null miprev\n");
        }
     }*/
    return(pindex);
}
...
bool ProcessNewBlock(bool from_miner,int32_t height,CValidationState &state, CNode* pfrom, CBlock* pblock, bool fForceProcessing, CDiskBlockPos *dbp)
{
    ...
    hash = pblock->GetHash();
    {
        LOCK(cs_main);
        if ( chainActive.Tip() != 0 )
            zkstake_currentheight_set(chainActive.Tip()->nHeight);
        checked = CheckBlock(&futureblock,height!=0?height:zkstake_block2height(pblock),0,*pblock, state, verifier,0);
        bool fRequested = MarkBlockAsReceived(hash);
        fRequested |= fForceProcessing;
        if ( checked != 0 && zkstake_checkPOW(0,0,pblock,height) < 0 ) //from_miner && ASSETCHAINS_STAKED == 0
        {
            checked = 0;
...
//void zkstake_pindex_init(CBlockIndex *pindex,int32_t height);
...
bool static LoadBlockIndexDB()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(uint256, CBlockIndex*)& item, mapBlockIndex)
    {
        ...
        //zkstake_pindex_init(pindex,(int32_t)pindex->nHeight);
    }
    ...
    if ( chainName.isKMD() ) {
        ...
        int32_t longestchain = zkstake_longestchain();
        // TODO: zkstake_longestchain does not have the data it needs at the time LoadBlockIndexDB
    ...
    if ( (pindex= chainActive.Tip()) != 0 )
    {
        if ( ASSETCHAINS_SAPLING <= 0 )
        {
            ...
            zkstake_activate_sapling(pindex);
        }
    }
    return true;
}
...
bool LoadBlockIndex(bool reindex)
{
    ...
    ZKSTAKE_LOADINGBLOCKS = true;
    if (!reindex && !LoadBlockIndexDB())
    {
        ZKSTAKE_LOADINGBLOCKS = false;
        return false;
    }
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                if (mapBlockIndex.count(hash) == 0 || (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA) == 0) {
                    ...
                    if (state.IsError())
                        break;
                } else if (hash != chainparams.GetConsensus().hashGenesisBlock && zkstake_blockheight(hash) % 1000 == 0) {
                    LogPrintf("Block Import: already had block %s at height %d\n", hash.ToString(), zkstake_blockheight(hash));
                }
...
#include "zkstake_nSPV_defs.h"
#include "zkstake_nSPV.h"            // shared defines, structs, serdes, purge functions
#include "zkstake_nSPV_fullnode.h"   // nSPV fullnode handling of the getnSPV request messages
#include "zkstake_nSPV_superlite.h"  // nSPV superlite client, issuing requests and handling nSPV responses
#include "zkstake_nSPV_wallet.h"     // nSPV_send and support functions, really all the rest is to support this
...
//LogPrintf("netmsg: %s\n", strCommand.c_str());
    ...
    {
        if ( ZKSTAKE_NSPV == 0 )//&& ZKSTAKE_INSYNC != 0 )
        {
            std::vector<uint8_t> payload;
            vRecv >> payload;
            zkstake_nSPVreq(pfrom,payload);
        }
        return(true);
    }
    else if (GetBoolArg("-nspv_msg", DEFAULT_NSPV_PROCESSING) && strCommand == "nSPV")
    {
        if ( ZKSTAKE_NSPV_SUPERLITE )
        {
            std::vector<uint8_t> payload;
            vRecv >> payload;
            zkstake_nSPVresp(pfrom,payload);
        }
        return(true);
    }
    else if ( ZKSTAKE_NSPV_SUPERLITE )
        return(true);
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        /*CBlockIndex *pindex;
        if ( zkstake_requestedhash != zero && zkstake_requestedcount < 16 && (pindex= zkstake_getblockindex(zkstake_requestedhash)) != 0 )
        {
            LogPrint("net","zkstake_requestedhash.%d request %s to nodeid.%d\n",zkstake_requestedcount,zkstake_requestedhash.ToString().c_str(),pto->GetId());
            LogPrintf("zkstake_requestedhash.%d request %s to nodeid.%d\n",zkstake_requestedcount,zkstake_requestedhash.ToString().c_str(),pto->GetId());
            vGetData.push_back(CInv(MSG_BLOCK, zkstake_requestedhash));
            MarkBlockAsInFlight(pto->GetId(), zkstake_requestedhash, consensusParams, pindex);
            zkstake_requestedcount++;
            if ( zkstake_requestedcount > 16 )
            {
                memset(&zkstake_requestedhash,0,sizeof(zkstake_requestedhash));
                zkstake_requestedcount = 0;
            }
