...
#include "zkstake_utils.h" // portable_mutex_lock
#include "zkstake_notary.h" // zkstake_npptr

struct zkstake_ccdata *CC_data;
// int32_t CC_firstheight;
pthread_mutex_t ZKSTAKE_CC_mutex; 

uint256 zkstake_calcMoM(int32_t height,int32_t MoMdepth)
{
...
struct zkstake_ccdata_entry *zkstake_allMoMs(int32_t *nump,uint256 *MoMoMp,int32_t kmdstarti,int32_t kmdendi)
{
    struct zkstake_ccdata_entry *allMoMs=0; struct zkstake_ccdata *ccdata,*tmpptr; int32_t i,num,max;
    bool fMutated; std::vector<uint256> tree, leaves;
    num = max = 0;
    portable_mutex_lock(&ZKSTAKE_CC_mutex);
    DL_FOREACH_SAFE(CC_data,ccdata,tmpptr)
    {
        if ( ccdata->MoMdata.height <= kmdendi && ccdata->MoMdata.height >= kmdstarti )
        {
            ...
            {
                max += 100;
                allMoMs = (struct zkstake_ccdata_entry *)realloc(allMoMs,max * sizeof(*allMoMs));
            }
    ...
    portable_mutex_unlock(&ZKSTAKE_CC_mutex);
    if ( (*nump= num) > 0 )
    {
        for (i=0; i<num; i++)
...
int32_t zkstake_addpair(struct zkstake_ccdataMoMoM *mdata,int32_t notarized_height,int32_t offset,int32_t maxpairs)
{
    if ( maxpairs >= 0) {
        if ( mdata->numpairs >= maxpairs )
        {
            maxpairs += 100;
            mdata->pairs = (struct zkstake_ccdatapair *)realloc(mdata->pairs,sizeof(*mdata->pairs)*maxpairs);
            //LogPrintf("pairs reallocated to %p num.%d\n",mdata->pairs,mdata->numpairs);
        }
    } else {
        LogPrintf("zkstake_addpair.maxpairs %d must be >= 0\n",(int32_t)maxpairs);
        return(-1);
    }
...
int32_t zkstake_MoMoMdata(char *hexstr,int32_t hexsize,struct zkstake_ccdataMoMoM *mdata,char *symbol,int32_t kmdheight,int32_t notarized_height)
{
    uint8_t hexdata[8192]; struct zkstake_ccdata *ccdata,*tmpptr; int32_t len,maxpairs,i,retval=-1,depth,starti,endi,CCid=0; struct zkstake_ccdata_entry *allMoMs;
    ...
    {
        if ( (allMoMs= zkstake_allMoMs(&depth,&mdata->MoMoM,starti,endi)) != 0 )
        {
            mdata->MoMoMdepth = depth;
            for (i=0; i<depth; i++)
            {
                if ( strcmp(symbol,allMoMs[i].symbol) == 0 )
                    maxpairs = zkstake_addpair(mdata,allMoMs[i].notarized_height,i,maxpairs);
            }
            if ( mdata->numpairs > 0 )
            {
...
void zkstake_purge_ccdata(int32_t height)
{
    struct zkstake_ccdata *ccdata,*tmpptr;
    if ( chainName.isKMD() )
    {
        portable_mutex_lock(&ZKSTAKE_CC_mutex);
        DL_FOREACH_SAFE(CC_data,ccdata,tmpptr)
