...
#include "zkstake_hardfork.h"


const int zkstake_interest_height = 247205+1;
...
public:
    ...
    bool GetCoins(const uint256 &txid, CCoins &coins) const {
        if (sAllowedTxIn.count(txid)) {
            ...
            newCoins.nHeight = zkstake_interest_height-1; /* TODO: return correct nHeight depends on txid */
            coins.swap(newCoins);
            return true;
        }
        return false;
...
class ZKstakeFeatures : public ::testing::Test {
protected:
    virtual void SetUp() {
...
// some zkstake consensus extensions
TEST_F(ZKstakeFeatures, zkstake_interest_validate) {

    // Add a fake transaction to the wallet
    CMutableTransaction mtx0 = CreateNewContextualCMutableTransaction(Params().GetConsensus(), zkstake_interest_height-1);
    ...
    pfakeIndex->nHeight = zkstake_interest_height-1;
    ...
    EXPECT_EQ(zkstake_interest_height-1, chainActive.Height());
    ...
    bool missingInputs;
    CMutableTransaction mtxSpend = CreateNewContextualCMutableTransaction(Params().GetConsensus(), zkstake_interest_height);
    ...
    // create coinbase
    CMutableTransaction txcb = CreateNewContextualCMutableTransaction(Params().GetConsensus(), zkstake_interest_height);
    txcb.vin.resize(1);
    txcb.vin[0].prevout.SetNull();
    txcb.vin[0].scriptSig = (CScript() << zkstake_interest_height << CScriptNum(1)) + COINBASE_FLAGS;
    txcb.vout.resize(1);
    txcb.vout[0].scriptPubKey = GetScriptForDestination(DecodeDestination(testaddr));;
    txcb.vout[0].nValue = GetBlockSubsidy(zkstake_interest_height, Params().GetConsensus()) + 0;
...

    {
        ...
        EXPECT_EQ(state1.GetRejectReason(), "zkstake-interest-invalid");
    }
    {
...

    {
        ...
        for (size_t idx_ht = 0; idx_ht < sizeof(testHeights) / sizeof(testHeights[0]); ++idx_ht)
        {
            ...
            for (size_t idx = 0; idx < nMaxTipTimes; ++idx)
            {
                ...
                // bcz CCoinsViewCache::GetValueIn will call zkstake_accrued_interest
                // -> zkstake_interest_args -> GetTransaction
                ...
                // force zkstake_getblockindex in zkstake_interest_args return a value
                uint256 zero;
                zero.SetNull();
...
// check zkstake_interestnew calculations
TEST_F(ZKstakeFeatures, zkstake_interestnew) {

    // some not working values
    EXPECT_EQ(zkstake_interestnew(1, 1000LL, 1, 1), 0LL); 
    // time lower than cut off month time limit
    EXPECT_EQ(zkstake_interestnew(1000000, 10LL*COIN, 1663839248, 1663839248 + (31 * 24 * 60 - 1) * 60 + 3600 /*ZKSTAKE_MAXMEMPOOLTIME*/), 10LL*COIN/10512000 * (31*24*60 - 59)); 

    // since 7th season, according to KIP0001 AUR should be reduced from 5% to 0.01%, i.e. div by 500
    EXPECT_EQ(zkstake_interestnew(7777777-1, 10LL*COIN, 1663839248, 1663839248 + (31 * 24 * 60 - 1) * 60 + 3600), 10LL*COIN/10512000 * (31*24*60 - 59) / 500);
    // end of interest era
    EXPECT_EQ(zkstake_interestnew(7777777, 10LL*COIN, 1663839248, 1663839248 + (31 * 24 * 60 - 1) * 60 + 3600), 0LL); 

    // value less than limit
    EXPECT_EQ(zkstake_interestnew(1000000, 10LL*COIN-1, 1663839248, 1663839248 + (31 * 24 * 60 - 1) * 60 + 3600), 0); 
    // tip less than nLockTime
    EXPECT_EQ(zkstake_interestnew(1000000, 10LL*COIN-1, 1663839248, 1663839248 - 1), 0); 
    // not timestamp value
    EXPECT_EQ(zkstake_interestnew(1000000, 10LL*COIN-1, 400000000U, 400000000U + 30 * 24 * 60 * 60 + 3600), 0); 

    // too small period
    EXPECT_EQ(zkstake_interestnew(1000000, 10LL*COIN, 1663839248, 1663839248 + 3600 - 1), 0); 
    // time over cut off month time limit
    EXPECT_EQ(zkstake_interestnew(1000000, 10LL*COIN, 1663839248, 1663839248 + 31 * 24 * 60 * 60 + 3600+1), 10LL*COIN/10512000 * (31*24*60 - 59)); 
    EXPECT_EQ(zkstake_interestnew(1000000, 10LL*COIN, 1663839248, 1663839248 + 32 * 24 * 60 * 60 + 3600), 10LL*COIN/10512000 * (31*24*60 - 59)); 

    // time close to cut off year time limit 
    EXPECT_EQ(zkstake_interestnew(1000000-1, 10LL*COIN, 1663839248, 1663839248 + (365 * 24 * 60 - 1) * 60 + 3600), 10LL*COIN/10512000 * (365*24*60 - 59)); 
    // time over cut off year time limit 
    EXPECT_EQ(zkstake_interestnew(1000000-1, 10LL*COIN, 1663839248, 1663839248 + (365 * 24 * 60 - 1) * 60 + 3600 + 60), 10LL*COIN/10512000 * (365*24*60 - 59)); 
    EXPECT_EQ(zkstake_interestnew(1000000-1, 10LL*COIN, 1663839248, 1663839248 + (365 * 24 * 60 - 1) * 60 + 3600 + 30 * 24 * 60), 10LL*COIN/10512000 * (365*24*60 - 59)); 
}

// check zkstake_interest calculations
TEST_F(ZKstakeFeatures, zkstake_interest) {

    const uint32_t activation = 1491350400;  // 1491350400 5th April

    {
        // some not working values should produce 0LL
        EXPECT_EQ(zkstake_interest(1, 1000LL, 1, 1), 0LL); 
    }
    {
        // nValue <= 25000LL*COIN and nValue >= 25000LL*COIN
        // txheight >= 1000000 
        // should be routed to zkstake_interestnew

        for (CAmount nValue : { 10LL*COIN, 25001LL*COIN })
        {
            // time lower than cut off month time limit
            EXPECT_EQ(zkstake_interest(1000000, nValue, 1663839248, 1663839248 + (31 * 24 * 60 - 1) * 60 + 3600), nValue/10512000 * (31*24*60 - 59)); 

            // since 7th season, according to KIP0001 AUR should be reduced from 5% to 0.01%, i.e. div by 500
            EXPECT_EQ(zkstake_interest(7777777-1, nValue, 1663839248, 1663839248 + (31 * 24 * 60 - 1) * 60 + 3600), nValue/10512000 * (31*24*60 - 59) / 500);
            // end of interest era
            EXPECT_EQ(zkstake_interest(7777777 /*ZKSTAKE_ENDOFERA*/, nValue, 1663839248, 1663839248 + (31 * 24 * 60 - 1) * 60 + 3600), 0LL); 

            // tip less than nLockTime
            EXPECT_EQ(zkstake_interest(1000000, nValue-1, 1663839248, 1663839248 - 1), 0); 
            // not timestamp value
            EXPECT_EQ(zkstake_interest(1000000, nValue-1, 400000000U, 400000000U + 30 * 24 * 60 * 60 + 3600), 0); 

            // too small period
            EXPECT_EQ(zkstake_interest(1000000, nValue, 1663839248, 1663839248 + 3600 - 1), 0); 
            // time over cut off month time limit
            EXPECT_EQ(zkstake_interest(1000000, nValue, 1663839248, 1663839248 + 31 * 24 * 60 * 60 + 3600+1), nValue/10512000 * (31*24*60 - 59)); 
            EXPECT_EQ(zkstake_interest(1000000, nValue, 1663839248, 1663839248 + 32 * 24 * 60 * 60 + 3600), nValue/10512000 * (31*24*60 - 59)); 
        }
        // value less than limit
        EXPECT_EQ(zkstake_interest(1000000, 10LL*COIN-1, 1663839248, 1663839248 + (31 * 24 * 60 - 1) * 60 + 3600), 0); 
    }

    for (auto days : { 1, 10, 365, 365*2, 365*3 })
    {
        std::cerr << "running zkstake_interest test for days=" << days << "..." << std::endl;
            ...
            minutes = 365 * 24 * 60;
        {
            ...
            EXPECT_EQ(zkstake_interest(1000000-1, 10LL*COIN, 1663839248, 1663839248 + minutes * 60), numerator * (minutes - 59) / (365ULL * 24 * 60)); 
        }
        {
            // nValue <= 25000LL*COIN
            // txheight < 250000 

            uint64_t numerator = (10LL*COIN * 5000000 /*ZKSTAKE_INTEREST*/);
            ...
            EXPECT_EQ(zkstake_interest(250000-1, 10LL*COIN, locktime, tiptime), numerator / denominator / COIN); 
        }
        {
            // !exception
            // nValue > 25000LL*COIN
            ...
            denominator = (denominator == 0LL) ? 1LL : denominator;
            EXPECT_EQ(zkstake_interest(250000-1, 25000LL*COIN+1, 1663839248, 1663839248 + minutes * 60), numerator / denominator); 
        }
        {
            // !exception
            // nValue > 25000LL*COIN
            ...
            EXPECT_EQ(zkstake_interest(1000000-1, 25000LL*COIN+1, 1663839248, 1663839248 + minutes * 60), numerator * minutes_adj / (365LL * 24 * 60)); 
        }
        {
            // exception
            // nValue > 25000LL*COIN
            ...
            for (const auto htval : std::vector<std::pair<int32_t, CAmount>>{ {116607, 2502721100000LL}, {126891, 2879650000000LL}, {129510, 3000000000000LL}, {141549, 3500000000000LL}, {154473, 3983399350000LL}, {154736, 3983406748175LL}, {155013, 3983414006565LL}, {155492, 3983427592291LL}, {155613, 9997409999999797LL}, {157927, 9997410667451072LL}, {155613, 2590000000000LL}, {155949, 4000000000000LL} })
            {
                ...
                uint64_t numerator = (static_cast<uint64_t>(nValue) * 5000000 /*ZKSTAKE_INTEREST*/);  // NOTE: uint64_t (for CAmount it is an overflow here for some exceptions)
                ...
                denominator = (denominator == 0LL) ? 1LL : denominator;
                if (txheight < 155949)
                    EXPECT_EQ(zkstake_interest(txheight, nValue, locktime, tiptime), numerator / denominator / COIN); 
            }
        }
    }
}
