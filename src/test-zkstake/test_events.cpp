...
{
    zkstake::event_kmdheight evt(10);
...
{
    zkstake::event_pricefeed evt(10);
    evt.num = 35;
...
{
    zkstake_state* state = zkstake_stateptrget((char*)symbol);
 ...
 */
TEST(test_events, zkstake_faststateinit_test)
{
    char symbol[] = "TST";
    chainName = assetchain("TST");
    ZKSTAKE_EXTERNAL_NOTARIES = 1;
    IS_ZKSTAKE_NOTARY = false;   // avoid calling zkstake_verifynotarization
    ...
    try
    {
        ...
        // pubkey record
        {
            // create a binary file that should be readable by zkstake
            ...
            clear_state(symbol); // clear zkstake_state*
            // attempt to read the file
            zkstake_state* state = zkstake_stateptrget((char*)symbol);
            ASSERT_TRUE(state != nullptr);
            char* dest = nullptr;   // not used for 'P'
            // attempt to read the file
            int32_t result = zkstake_faststateinit( state, full_filename.c_str(), symbol, dest);
            // compare results
            EXPECT_EQ(result, 1);
            /* old way
            // The first and second event should be pub keys
            EXPECT_EQ(state->ZKstake_numevents, 1);
            zkstake_event* ev1 = state->ZKstake_events[0];
            ...
            */
            ...
            zkstake::event_pubkeys& ev2 = 
                    static_cast<zkstake::event_pubkeys&>( *state->events.front() );
            EXPECT_EQ(ev2.height, 10);
            EXPECT_EQ(ev2.type, zkstake::zkstake_event_type::EVENT_PUBKEYS);
        ...
        // notarized record
        {
            ...
            EXPECT_EQ(result, 1);
            /* old way
            EXPECT_EQ(state->ZKstake_numevents, 2);
            zkstake_event* ev = state->ZKstake_events[1];
            ...
            */
            ...
            zkstake::event_notarized& ev2 = 
                    static_cast<zkstake::event_notarized&>( *state->events.front() );
            EXPECT_EQ(ev2.height, 10);
            EXPECT_EQ(ev2.type, zkstake::zkstake_event_type::EVENT_NOTARIZED);
            // the serialized version should match the input
            EXPECT_TRUE(compare_serialization(full_filename, ev2));

            // check last nota read from zkstake state:
        ...
        // notarized M record
        {
            ...
            EXPECT_EQ(result, 1);
            /* old way
            EXPECT_EQ(state->ZKstake_numevents, 3);
            zkstake_event* ev = state->ZKstake_events[2];
            EXPECT_EQ(ev->height, 1);
            ...
            */
            ...
            //std::advance(itr, 2);
            zkstake::event_notarized& ev2 = static_cast<zkstake::event_notarized&>( *state->events.front() );
            EXPECT_EQ(ev2.height, 10);
            EXPECT_EQ(ev2.type, zkstake::zkstake_event_type::EVENT_NOTARIZED);
            // the serialized version should match the input
            EXPECT_TRUE(compare_serialization(full_filename, ev2));
        ...
        // record type "U" (deprecated)
        {
            ...
            EXPECT_EQ(result, 1);
            /* old way
            EXPECT_EQ(state->ZKstake_numevents, 3); // does not get added to state
            */
        ...
        // record type K (KMD height)
        {
            ...
            EXPECT_EQ(result, 1);
            /* old way
            EXPECT_EQ(state->ZKstake_numevents, 4);
            zkstake_event* ev = state->ZKstake_events[3];
            EXPECT_EQ(ev->height, 1);
            EXPECT_EQ(ev->type, 'K');            
            */
            ASSERT_TRUE(state->events.size() != 0); // prevent test crash
            ...
            //auto itr = state->events.begin();
            //std::advance(itr, 3);
            zkstake::event_kmdheight& ev2 = static_cast<zkstake::event_kmdheight&>( *state->events.front() );
            EXPECT_EQ(ev2.height, 10);
            EXPECT_EQ(ev2.type, zkstake::zkstake_event_type::EVENT_KMDHEIGHT);
            // the serialized version should match the input
            EXPECT_TRUE(compare_serialization(full_filename, ev2));
        }
        // record type T (KMD height with timestamp)
        {
            ...
            EXPECT_EQ(result, 1);
            /* old way
            EXPECT_EQ(state->ZKstake_numevents, 5);
            zkstake_event* ev = state->ZKstake_events[4];
            EXPECT_EQ(ev->height, 1);
        ...
        // record type R (opreturn)
        {
            ...
            EXPECT_EQ(result, 1);
            /* old way
            EXPECT_EQ(state->ZKstake_numevents, 6);
            zkstake_event* ev = state->ZKstake_events[5];
            EXPECT_EQ(ev->height, 1);
            EXPECT_EQ(ev->type, 'R');
            */
            ASSERT_TRUE(state->events.size() != 0); // prevent test crash
            ...
            auto itr = state->events.begin();
            std::advance(itr, 5);
            zkstake::event_opreturn& ev2 = static_cast<zkstake::event_opreturn&>( *state->events.front() );
            EXPECT_EQ(ev2.height, 10);
            EXPECT_EQ(ev2.type, zkstake::zkstake_event_type::EVENT_OPRETURN);
            // the serialized version should match the input
            EXPECT_TRUE(compare_serialization(full_filename, ev2));
        }
        // record type V
        {
            ...
            EXPECT_EQ(result, 1);
            /* old way
            EXPECT_EQ(state->ZKstake_numevents, 7);
            zkstake_event* ev = state->ZKstake_events[6];
            EXPECT_EQ(ev->height, 1);
            EXPECT_EQ(ev->type, 'V');
            */
            ASSERT_TRUE(state->events.size() != 0); // prevent test crash
            ...
            //auto itr = state->events.begin();
            // std::advance(itr, 6);
            zkstake::event_pricefeed& ev2 = static_cast<zkstake::event_pricefeed&>( *state->events.front() );
            EXPECT_EQ(ev2.height, 10);
            EXPECT_EQ(ev2.type, zkstake::zkstake_event_type::EVENT_PRICEFEED);
            // the serialized version should match the input
            EXPECT_TRUE(compare_serialization(full_filename, ev2));
        }
        ...
        // all together in 1 file
        {
            ...
            EXPECT_EQ(result, 1);
            /* old way
            EXPECT_EQ(state->ZKstake_numevents, 14);
            zkstake_event* ev1 = state->ZKstake_events[7];
            EXPECT_EQ(ev1->height, 1);
            EXPECT_EQ(ev1->type, 'P');
            */
            ASSERT_TRUE(state->events.size() != 0); // prevent test crash
            ...
            std::advance(itr, 0);
            {
                ...
                EXPECT_EQ( (**itr).type, zkstake::zkstake_event_type::EVENT_PUBKEYS);
                itr++;
            }
            {
                EXPECT_EQ( (**itr).height, 10);
                EXPECT_EQ( (**itr).type, zkstake::zkstake_event_type::EVENT_NOTARIZED);
                itr++;
            }
            {
                EXPECT_EQ( (**itr).height, 10);
            ...
            {
                EXPECT_EQ( (**itr).height, 10);
                EXPECT_EQ( (**itr).type, zkstake::zkstake_event_type::EVENT_KMDHEIGHT);
                itr++;
            }
            {
                EXPECT_EQ( (**itr).height, 10);
            ...
            {
                EXPECT_EQ( (**itr).height, 10);
                EXPECT_EQ( (**itr).type, zkstake::zkstake_event_type::EVENT_OPRETURN);
                itr++;
            }
            {
                EXPECT_EQ( (**itr).height, 10);
                EXPECT_EQ( (**itr).type, zkstake::zkstake_event_type::EVENT_PRICEFEED);
                itr++;
            }
        }
    } 
...
// same test for KMD
TEST(test_events, zkstake_faststateinit_test_kmd)
{
    // Nothing should be added to events if this is the zkstake chain

    char symbol[] = "KMD";
    chainName = assetchain();
    ZKSTAKE_EXTERNAL_NOTARIES = 0;
    IS_ZKSTAKE_NOTARY = false;   // avoid calling zkstake_verifynotarization
    ...
    {
        {
            ...
            // check that the new way is the same
            EXPECT_EQ(state->events.size(), 1);
            zkstake::event_pubkeys& ev = static_cast<zkstake::event_pubkeys&>( *state->events.front() );
            EXPECT_EQ(ev.height, 10);
            EXPECT_EQ(ev.type, zkstake::zkstake_event_type::EVENT_PUBKEYS);
            std::fclose(fp);
            // verify files still exists
            EXPECT_TRUE(boost::filesystem::exists(full_filename));
        }
...
{
    // make an object
    zkstake::event_pubkeys pk1(1);
    pk1.num = 2;
    memset(pk1.pubkeys[0], 1, 33);
    memset(pk1.pubkeys[1], 2, 33);
    // make a copy
    std::vector<std::shared_ptr<zkstake::event>> events;
    events.push_back( std::make_shared<zkstake::event_pubkeys>(pk1) );
    zkstake::event_pubkeys& pk2 = static_cast<zkstake::event_pubkeys&>(*events.front());
    // are they equal?
