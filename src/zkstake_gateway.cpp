...
#include "zkstake_utils.h" // zkstake_stateptrget
#include "zkstake_bitcoind.h" // zkstake_checkcommission
 ...
 */
bool zkstake_checkvout(int32_t vout,int32_t k,int32_t indallvouts)
{
 ...
 */
int32_t zkstake_bannedset(int32_t *indallvoutsp,uint256 *array,int32_t max)
{
    if ( sizeof(banned_txids)/sizeof(*banned_txids) > max )
    {
        LogPrintf("zkstake_bannedset: buffer too small %d vs %d\n",(int32_t)(sizeof(banned_txids)/sizeof(*banned_txids)),max);
        StartShutdown();
    }
    int32_t i;
 ...
 */
int32_t zkstake_check_deposit(int32_t height,const CBlock& block)
{
            ...
            for (int32_t j=0; j<n; j++) // for each vin
            {
                for (int32_t k=0; k<numbanned; k++) // for each banned txid
                {
                    if ( block.vtx[i].vin[j].prevout.hash == array[k] 
                            && zkstake_checkvout(block.vtx[i].vin[j].prevout.n,k,indallvouts) )
                    {
         ...
         NetworkUpgradeActive(height, Params().GetConsensus(), Consensus::UPGRADE_SAPLING) )
    {
        ...
        if ( chainName.isKMD() )
        {
            ...
            else if ( block.nBits == ZKSTAKE_MINDIFF_NBITS && total > 0 ) // to deal with fee stealing
            {
                LogPrintf("notary mined ht.%d with extra %.8f\n",height,dstr(total));
                if ( height > ZKSTAKE_NOTARIES_HEIGHT1 )
                    return(-1);
            }
            ...
            else if ( height > 814000 )
            {
                ...
                int32_t num;
                return(-1 * (zkstake_electednotary(&num,script+1,height,0) >= 0) * (height > 1000000));
            }
        }
        else
        {
            ...
            {
                if ( (checktoshis= zkstake_checkcommission((CBlock *)&block,height)) < 0 )
                {
...
void zkstake_stateind_set(struct zkstake_state *sp,uint32_t *inds,int32_t n,uint8_t *filedata,long datalen,const char *symbol,const char *dest)
{
        ...
        for (lastfpos=fpos=prevpos100=i=0; i<n; i++)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if ( doissue != 0 )
                    {
                        zkstake_parsestatefiledata(sp,filedata,&lastfpos,datalen,symbol,dest);
                        count++;
                    }
                }
            }
...
/**
 * @brief Validate the index of the zkstakestate file
 * 
 * @param[in] sp the zkstake_state struct
 ...
 * @return -1 on error
 */
long zkstake_stateind_validate(struct zkstake_state *sp,const std::string& indfname,uint8_t *filedata,long datalen,
    ...
    if ( (inds= OS_fileptr(&fsize,indfname.c_str())) != 0 )
    {
        ...
        if ( (fsize % sizeof(uint32_t)) == 0 )
        {
            ...
            *prevpos100p = prevpos100;
            if ( sp != 0 )
                zkstake_stateind_set(sp,(uint32_t *)inds,n,filedata,fpos,symbol,dest);
            free(inds);
            return fpos;
        } 
        else 
...
long zkstake_indfile_update(FILE *indfp,uint32_t *prevpos100p,long lastfpos,long newfpos,uint8_t func,uint32_t *indcounterp)
{
    if ( indfp != 0 )
    {
...
/***
 * @brief read the zkstakestate file
 * @param sp the zkstake_state struct
 * @param fname the filename
 * @param symbol the chain symbol
 * @param dest the "parent" chain
 * @return true on success
 */
bool zkstake_faststateinit(zkstake_state *sp,const char *fname,char *symbol, const char *dest)
{
    ...
    if ( (filedata= OS_fileptr(&datalen,fname)) != 0 )
    {
        ...
        int32_t func;
        while (!ShutdownRequested() && (func= zkstake_parsestatefiledata(sp,filedata,&fpos,datalen,symbol,dest)) >= 0)
        {
            lastfpos = zkstake_indfile_update(indfp,&prevpos100,lastfpos,fpos,func,&indcounter);
        }
        if (ShutdownRequested()) { fclose(indfp); return false; }
        if ( indfp != nullptr )
        {
            fclose(indfp);
            if ( (fpos= zkstake_stateind_validate(0,indfname,filedata,datalen,&prevpos100,&indcounter,symbol,dest)) < 0 )
                LogPrintf("unexpected zkstakestate.ind validate failure %s datalen.%ld\n",indfname.c_str(),datalen);
            else 
...
//uint64_t zkstake_interestsum(); // in wallet/rpcwallet.cpp

/***
 * @brief update wallet balance / interest
 * @note called only on KMD chain every 10 seconds ( see ThreadUpdateZKstakeInternals() )
 */
void zkstake_update_interest()
{
    static uint32_t lastinterest; // prevent needless zkstake_interestsum calls
    if (zkstake_chainactive_timestamp() > lastinterest)
    {
        zkstake_interestsum();
        lastinterest = zkstake_chainactive_timestamp();
    }
