...
#ifndef ZKSTAKE_NSPVSUPERLITE_H
#define ZKSTAKE_NSPVSUPERLITE_H
...
// zkstake_nSPVresp is called from async message processing

void zkstake_nSPVresp(CNode *pfrom,std::vector<uint8_t> response) // received a response
{
...
// zkstake_nSPV from main polling loop (really this belongs in its own file, but it is so small, it ended up here)

void zkstake_nSPV(CNode *pto) // polling loop from SendMessages
{
...
UniValue NSPV_getinfo_json(struct NSPV_inforesp *ptr)
{
    ...
    result.push_back(Pair("result","success"));
    result.push_back(Pair("nSPV",ZKSTAKE_NSPV==-1?"disabled":(ZKSTAKE_NSPV_SUPERLITE?"superlite":"fullnode")));
