...
#include "zkstake.h" // zkstake_stateupdate()
#include "zkstake_structs.h" // ZKSTAKE_NOTARIES_HARDCODED
#include "zkstake_utils.h" // zkstake_stateptr
#include "zkstake_bitcoind.h"

//struct knotaries_entry *Pubkeys;  // todo remove

// statics used within this .cpp for caching purposes
static int didinit; // see zkstake_init
static uint8_t kmd_pubkeys[NUM_KMD_SEASONS][64][33]; // see zkstake_notaries
static int32_t hwmheight; // highest height ever passed to zkstake_notariesinit
static int32_t hadnotarization; // used in zkstake_dpowconfs
static bool didinit_NOTARIES[NUM_KMD_SEASONS]; // zkstake_notaries()
...
{
    int32_t htind = height / ZKSTAKE_ELECTION_GAP;
    if ( htind >= ZKSTAKE_MAXBLOCKS / ZKSTAKE_ELECTION_GAP )
        htind = (ZKSTAKE_MAXBLOCKS / ZKSTAKE_ELECTION_GAP) - 1;
    return htind;
}
...
int32_t zkstake_isnotaryvout(char *coinaddr,uint32_t tiptime) // from ac_private chains only
{
    int32_t season = getacseason(tiptime);
    if ( NOTARY_ADDRESSES[season-1][0][0] == 0 )
    {
        uint8_t pubkeys[64][33];
        zkstake_notaries(pubkeys,0,tiptime);
    }
...
int32_t zkstake_notaries(uint8_t pubkeys[64][33],int32_t height,uint32_t timestamp)
{
    ...
    if ( chainName.isKMD() )   
        ...
        timestamp = zkstake_heightstamp(height); // derive the timestamp from the passed-in height

    // If this chain is not a staked chain, use the normal ZKstake logic to determine notaries. 
    ...
    if ( is_STAKED(chainName.symbol()) == 0 )
    {
        ...
        if ( chainName.isKMD() )
        {
            ...
            if ( height >= ZKSTAKE_NOTARIES_HARDCODED )
    ...
    std::lock_guard<std::mutex> lock(zkstake_mutex);
...
int32_t zkstake_electednotary(int32_t *numnotariesp,uint8_t *pubkey33,int32_t height,uint32_t timestamp)
{
    int32_t i,n; uint8_t pubkeys[64][33];
    n = zkstake_notaries(pubkeys,height,timestamp);
    *numnotariesp = n;
    for (i=0; i<n; i++)
    {
...
int32_t zkstake_ratify_threshold(int32_t height,uint64_t signedmask)
{
 ...
 */
void zkstake_notarysinit(int32_t origheight,uint8_t pubkeys[64][33],int32_t num)
{
    if ( Pubkeys == 0 )
        Pubkeys = (knotaries_entry *)calloc(1 + (ZKSTAKE_MAXBLOCKS / ZKSTAKE_ELECTION_GAP),sizeof(*Pubkeys));
    knotaries_entry N;
    memset(&N,0,sizeof(N));
    ...
    if ( origheight > 0 )
    {
        int32_t height = (origheight + ZKSTAKE_ELECTION_GAP/2);
        height /= ZKSTAKE_ELECTION_GAP;
        height = ((height + 1) * ZKSTAKE_ELECTION_GAP);
    ...
    }
    {
        ...
        N.numnotaries = num;
        for (int32_t i=htind; i<ZKSTAKE_MAXBLOCKS / ZKSTAKE_ELECTION_GAP; i++)
        {
            ...
            Pubkeys[i] = N;
            Pubkeys[i].height = i * ZKSTAKE_ELECTION_GAP;
        }
    }
...
int32_t zkstake_chosennotary(int32_t *notaryidp,int32_t height,uint8_t *pubkey33,uint32_t timestamp)
{
    ...
    *notaryidp = -1;
    if ( height < 0 )//|| height >= ZKSTAKE_MAXBLOCKS )
    {
        LogPrintf("zkstake_chosennotary ht.%d illegal\n",height);
        return(-1);
    }
    if ( height >= ZKSTAKE_NOTARIES_HARDCODED || !chainName.isKMD() )
    {
        if ( (*notaryidp= zkstake_electednotary(&numnotaries,pubkey33,height,timestamp)) >= 0 && numnotaries != 0 )
        {
 ...
 */
const notarized_checkpoint *zkstake_npptr(int32_t height)
{
    char symbol[ZKSTAKE_ASSETCHAIN_MAXLEN];
    char dest[ZKSTAKE_ASSETCHAIN_MAXLEN]; 

    zkstake_state *sp = zkstake_stateptr(symbol, dest);
    if ( sp != nullptr )
    {
 ...
 */
int32_t zkstake_notarized_height(int32_t *prevMoMheightp,uint256 *hashp,uint256 *txidp)
{
    char symbol[ZKSTAKE_ASSETCHAIN_MAXLEN];
    char dest[ZKSTAKE_ASSETCHAIN_MAXLEN];
...
int32_t zkstake_dpowconfs(int32_t txheight,int32_t numconfs)
{
    ...
    if ( ZKSTAKE_DPOWCONFS != 0 && txheight > 0 && numconfs > 0 && (sp= zkstake_stateptr(symbol,dest)) != nullptr )
    {
...
int32_t zkstake_MoMdata(int32_t *notarized_htp,uint256 *MoMp,uint256 *kmdtxidp,int32_t height,uint256 *MoMoMp,int32_t *MoMoMoffsetp,int32_t *MoMoMdepthp,int32_t *kmdstartip,int32_t *kmdendip)
{
    const notarized_checkpoint *np = zkstake_npptr(height);
    if ( np != nullptr )
    {
 ...
 * @returns the notarized height
 */
int32_t zkstake_notarizeddata(int32_t nHeight,uint256 *notarized_hashp,uint256 *notarized_desttxidp)
{
    char symbol[ZKSTAKE_ASSETCHAIN_MAXLEN];
    char dest[ZKSTAKE_ASSETCHAIN_MAXLEN];
...
/***
 * Add a notarized checkpoint to the zkstake_state
 * @param[in] sp the zkstake_state to add to
 ...
 */
void zkstake_notarized_update(struct zkstake_state *sp,int32_t nHeight,int32_t notarized_height,
        uint256 notarized_hash,uint256 notarized_desttxid,uint256 MoM,int32_t MoMdepth)
{
    if ( notarized_height >= nHeight )
    {
        LogPrintf("zkstake_notarized_update REJECT notarized_height %d > %d nHeight\n",notarized_height,nHeight);
        return;
    }
 ...
 */
void zkstake_init(int32_t height)
{
    uint256 zero; 
    uint8_t pubkeys[64][33];
    ...
    if ( !didinit )
    {
        ...
        if ( height >= 0 )
        {
            ...
            zkstake_notarysinit(0,pubkeys,count);
        }
        didinit = true;
        zkstake_stateupdate(0,0,0,0,zero,0,0,0,0,0,0,0,0,zero,0);
    }
}

/****
 * A nasty hack to reset statics in this file.
 * Do not put this in zkstake_notary.h, as it should only be used for testing
 */
void zkstake_notaries_uninit()
{
    didinit = 0;
    hwmheight = 0;
    hadnotarization = 0;
