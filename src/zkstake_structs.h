...
#define ZKSTAKE_MINRATIFY ((height < 90000) ? 7 : 11)
#define ZKSTAKE_NOTARIES_HARDCODED 180000 // DONT CHANGE Below this height notaries were hardcoded
#define ZKSTAKE_MAXBLOCKS 250000 // DONT CHANGE
...
//extern std::mutex zkstake_mutex;  //todo remove

struct zkstake_event
{
    struct zkstake_event *related;
    uint16_t len;
    int32_t height;
...
{
public:
    event(zkstake_event_type t, int32_t height) : type(t), height(height) {}
    virtual ~event() = default;
    zkstake_event_type type;
    int32_t height;
};
...
{
    event_rewind() : event(zkstake_event_type::EVENT_REWIND, 0) {}
...
{
    event_notarized() : event(zkstake_event_type::EVENT_NOTARIZED, 0), notarizedheight(0), MoMdepth(0) {
        memset(this->dest, 0, sizeof(this->dest));
    }
    event_notarized(int32_t ht, const char* _dest) : event(zkstake_event_type::EVENT_NOTARIZED, ht), notarizedheight(0), MoMdepth(0) {
...
struct zkstake_ccdataMoM
{
    uint256 MoM;
    int32_t MoMdepth,notarized_height,height,txi;
};

struct zkstake_ccdata_entry { uint256 MoM; int32_t notarized_height,kmdheight,txi; char symbol[65]; };
struct zkstake_ccdatapair { int32_t notarized_height,MoMoMoffset; };

struct zkstake_ccdataMoMoM
{
    uint256 MoMoM;
    int32_t kmdstarti,kmdendi,MoMoMoffset,MoMoMdepth,numpairs,len;
    struct zkstake_ccdatapair *pairs;
};

struct zkstake_ccdata
{
    struct zkstake_ccdata *next,*prev;
    struct zkstake_ccdataMoM MoMdata;
    uint32_t CCid,len;
    char symbol[65];
};
...
public:
    ...
    uint64_t redeemed;
    uint64_t shorted;
    std::list<std::shared_ptr<zkstake::event>> events;
