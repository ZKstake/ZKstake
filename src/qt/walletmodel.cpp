...
extern uint64_t zkstake_interestsum();
...
{
    return (chainName.isKMD()) ? zkstake_interestsum() : 0;
}
...
void WalletModel::checkBalanceChanged()
{
    ...
    CAmount newinterestBalance = (chainName.isKMD()) ? zkstake_interestsum() : 0;
    if (haveWatchOnly())
    {
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl& coinControl)
{
    ...
    for (const SendCoinsRecipient &rcp : recipients)
    {
        ...
        #endif
        {   // User-entered zkstake address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        for (const SendCoinsRecipient &rcp : transaction.getRecipients())
        {
            ...
            #endif 
                if (!rcp.message.isEmpty()) // Message from normal zkstake:URI (zkstake:123...?message=example)
