    ...
    signalled(false)
{
    ...
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the ZKstake block chain.").arg(tr(PACKAGE_NAME)) + " " +
...
//    dataDir = settings.value("strDataDir", dataDir).toString();
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || settings.value("fReset", false).toBool() || GetBoolArg("-resetguisettings", false))
    {
        ...
        intro.setWindowIcon(QIcon(":icons/zkstake"));
     ...
     * override -datadir in the zkstake.conf file in the default data directory
     * (to be consistent with zkstaked behavior)
     */
    if(dataDir != getDefaultDataDirectory())
