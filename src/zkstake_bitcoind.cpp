...
#include "zkstake.h" // zkstake_voutupdate()
#include "zkstake_utils.h" // OS_milliseconds
...
char *zkstake_issuemethod(char *userpass,char *method,char *params,uint16_t port)
{
    //static void *cHandle;
...
int32_t notarizedtxid_height(char *dest,char *txidstr,int32_t *kmdnotarized_heightp)
{
    ...
    *kmdnotarized_heightp = 0;
    ...
    if ( userpass[0] != 0 )
    {
        ...
        {
            if ( (jsonstr= zkstake_issuemethod(userpass,(char *)"getinfo",params,port)) != 0 )
            {
                //LogPrintf("(%s)\n",jsonstr);
            ...
            if ( (jsonstr= zkstake_issuemethod(userpass,(char *)"getblockchaininfo",params,port)) != 0 )
            {
                //LogPrintf("(%s)\n",jsonstr);
                if ( (json= cJSON_Parse(jsonstr)) != 0 )
                {
        ...
        if ( (jsonstr= zkstake_issuemethod(userpass,(char *)"getrawtransaction",params,port)) != 0 )
        {
            //LogPrintf("(%s)\n",jsonstr);
            if ( (json= cJSON_Parse(jsonstr)) != 0 )
            {
...
int32_t zkstake_verifynotarizedscript(int32_t height,uint8_t *script,int32_t len,uint256 NOTARIZED_HASH)
{
...
void zkstake_reconsiderblock(uint256 blockhash)
{
    char params[256],*jsonstr,*hexstr;
    sprintf(params,"[\"%s\"]",blockhash.ToString().c_str());
    if ( (jsonstr= zkstake_issuemethod(ASSETCHAINS_USERPASS,(char *)"reconsiderblock",params,ASSETCHAINS_RPCPORT)) != 0 )
    {
        //LogPrintf("zkstake_reconsiderblock.(%s) (%s %u) -> (%s)\n",params,ASSETCHAINS_USERPASS,ASSETCHAINS_RPCPORT,jsonstr);
        free(jsonstr);
    }
    //LogPrintf("zkstake_reconsiderblock.(%s) (%s %u) -> NULL\n",params,ASSETCHAINS_USERPASS,ASSETCHAINS_RPCPORT);
}

int32_t zkstake_verifynotarization(const char *symbol,const char *dest,int32_t height,int32_t NOTARIZED_HEIGHT,uint256 NOTARIZED_HASH,uint256 NOTARIZED_DESTTXID)
{
    char params[256];
    ...
    if ( strcmp(dest,"KMD") == 0 )
    {
        if ( ZKSBUSERPASS[0] != 0 )
        {
            if ( !chainName.isKMD() )
            {
                jsonstr = zkstake_issuemethod(ZKSBUSERPASS,(char *)"getrawtransaction",params,ZKSB_PORT);
                //LogPrintf("userpass.(%s) got (%s)\n",ZKSBUSERPASS,jsonstr);
            }
    ...
    else if ( strcmp(dest,"BTC") == 0 )     // Note: this should work for LTC too (BTC is used as an alias for LTC)
    {
        if ( BTCUSERPASS[0] != 0 )
        {
            jsonstr = zkstake_issuemethod(BTCUSERPASS,(char *)"getrawtransaction",params,DEST_PORT);
        }
        //else jsonstr = _dex_getrawtransaction();
        else return(0);
    }
            ...
            if ( (txjson= jobj(json,(char *)"result")) != 0 && (vouts= jarray(&n,txjson,(char *)"vout")) != nullptr )
            {
                ...
                if ( (skey= jobj(vout,(char *)"scriptPubKey")) != 0 )
                {
                    if ( (hexstr= jstr(skey,(char *)"hex")) != 0 )
                    {
                        ...
                        retval = zkstake_verifynotarizedscript(height,script,len,NOTARIZED_HASH);
                    }
                }
            }
            free_json(txjson);
...
CScript zkstake_makeopret(CBlock *pblock, bool fNew)
{
...
uint64_t zkstake_seed(int32_t height)
{
    uint64_t seed = 0;
    /*if ( 0 ) // problem during init time, seeds are needed for loading blockindex, so null seeds...
     ...
     hash = _zkstake_getblockhash(height);
     if ( memcmp(&hash,&zero,sizeof(hash)) == 0 )
     hash = zkstake_getblockhash(height);
     int32_t i;
     for (i=0; i<32; i++)
     LogPrintf("%02x",((uint8_t *)&hash)[i]);
     LogPrintf(" seed.%d\n",height);
...
uint32_t zkstake_txtime(CScript &opret,uint64_t *valuep,uint256 hash, int32_t n, char *destaddr)
{
...
uint32_t zkstake_txtime2(uint64_t *valuep,uint256 hash,int32_t n,char *destaddr)
{
    ...
    *valuep = 0;
    ...
    if ( (pindex= zkstake_getblockindex(hashBlock)) != 0 )
        txtime = pindex->nTime;
...
{
    if ( timestamp > nStakedDecemberHardforkTimestamp || zkstake_heightstamp(height) > nStakedDecemberHardforkTimestamp ) //December 2019 hardfork
        return(1);
    else return(0);
}

int32_t zkstake_hasOpRet(int32_t height, uint32_t timestamp)
{    
    return zkstake_newStakerActive(height, timestamp) == 1;
}

bool zkstake_checkopret(CBlock *pblock, CScript &merkleroot)
{
    merkleroot = pblock->vtx.back().vout.back().scriptPubKey;
    return(merkleroot.IsOpReturn() && merkleroot == zkstake_makeopret(pblock, false));
}
...
uint256 zkstake_calcmerkleroot(CBlock *pblock, uint256 prevBlockHash, int32_t nHeight, bool fNew, CScript scriptPubKey)
{
    std::vector<uint256> vLeaves;
    // rereate coinbase tx
...
int32_t zkstake_isPoS(CBlock *pblock, int32_t height,CTxDestination *addressout)
{
    ...
    if ( ASSETCHAINS_STAKED != 0 )
    {
        ...
        if ( n > 1 && pblock->vtx[n-1].vin.size() == 1 && pblock->vtx[n-1].vout.size() == 1+zkstake_hasOpRet(height,pblock->nTime) )
        {
            txid = pblock->vtx[n-1].vin[0].prevout.hash;
            vout = pblock->vtx[n-1].vin[0].prevout.n;
            txtime = zkstake_txtime(opret,&value,txid,vout,destaddr);
            if ( ExtractDestination(pblock->vtx[n-1].vout[0].scriptPubKey,voutaddress) )
            {
                ...
                if ( zkstake_newStakerActive(height, pblock->nTime) != 0 ) 
                {
                    if ( DecodeStakingOpRet(pblock->vtx[n-1].vout[1].scriptPubKey, merkleroot) != 0 && zkstake_calcmerkleroot(pblock, pblock->hashPrevBlock, height, false, pblock->vtx[0].vout[0].scriptPubKey) == merkleroot )
                    {
                        return(1);
                    }
                }
...
int32_t zkstake_is_notarytx(const CTransaction& tx)
{
...
int32_t zkstake_block2height(CBlock *block)
{
...
bool zkstake_block2pubkey33(uint8_t *pubkey33,CBlock *block)
{
    int32_t n;
    if ( !ZKSTAKE_LOADINGBLOCKS )
...
uint32_t zkstake_heightstamp(int32_t height)
{
    CBlockIndex *ptr;
    if ( height > 0 && (ptr= zkstake_chainactive(height)) != nullptr )
        return(ptr->nTime);
    return 0;
}

void zkstake_index2pubkey33(uint8_t *pubkey33,CBlockIndex *pindex,int32_t height)
{
    int32_t num,i; CBlock block;
    memset(pubkey33,0,33);
    if ( pindex != 0 )
    {
        if ( zkstake_blockload(block,pindex) == 0 )
            zkstake_block2pubkey33(pubkey33,&block);
    }
}

int32_t zkstake_eligiblenotary(uint8_t pubkeys[66][33],int32_t *mids,uint32_t blocktimes[66],int32_t *nonzpkeysp,int32_t height)
{
    ...
    n = zkstake_notaries(notarypubs33,height,0);
    for (i=duplicate=0; i<66; i++)
    {
        if ( (pindex= zkstake_chainactive(height-i)) != 0 )
        {
            blocktimes[i] = pindex->nTime;
            if ( zkstake_blockload(block,pindex) == 0 )
            {
                zkstake_block2pubkey33(pubkeys[i],&block);
                for (j=0; j<n; j++)
                {
...
int32_t zkstake_minerids(uint8_t *minerids,int32_t height,int32_t width)
{
    ...
    numnotaries = zkstake_notaries(notarypubs33,height,0);
    for (i=nonz=0; i<width; i++)
    {
        if ( height-i <= 0 )
            continue;
        if ( (pindex= zkstake_chainactive(height-width+i+1)) != 0 )
        {
            if ( zkstake_blockload(block,pindex) == 0 )
            {
                zkstake_block2pubkey33(pubkey33,&block);
...
int32_t zkstake_is_special(uint8_t pubkeys[66][33],int32_t mids[66],uint32_t blocktimes[66],int32_t height,uint8_t pubkey33[33],uint32_t blocktime)
{
    int32_t i,j,notaryid=0,minerid,limit,nid; uint8_t destpubkey33[33];
    zkstake_chosennotary(&notaryid,height,pubkey33,blocktimes[0]);
    if ( height >= 82000 )
    {
        if ( notaryid >= 0 )
        {
    ...
    else
    {
        if ( height >= 34000 && notaryid >= 0 )
        {
            ...
            for (i=1; i<limit; i++)
            {
                zkstake_chosennotary(&nid,height-i,pubkey33,blocktimes[i]);
                if ( nid == notaryid )
                {
                    //for (j=0; j<66; j++)
                    //    LogPrintf("%d ",mids[j]);
...
int32_t zkstake_MoM(int32_t *notarized_heightp,uint256 *MoMp,uint256 *kmdtxidp,int32_t nHeight,uint256 *MoMoMp,int32_t *MoMoMoffsetp,int32_t *MoMoMdepthp,int32_t *kmdstartip,int32_t *kmdendip)
{
    int32_t depth,notarized_ht; uint256 MoM,kmdtxid;
    depth = zkstake_MoMdata(&notarized_ht,&MoM,&kmdtxid,nHeight,MoMoMp,MoMoMoffsetp,MoMoMdepthp,kmdstartip,kmdendip);
 ...
 */
bool zkstake_checkpoint(int32_t *notarized_heightp, int32_t nHeight, uint256 hash)
{
    CBlockIndex *pindex;
    ...
    int32_t notarized_height = zkstake_notarizeddata(pindex->nHeight,&notarized_hash,&notarized_desttxid);
    *notarized_heightp = notarized_height;
    ...
    {
        //verify that the block info returned from zkstake_notarizeddata matches the actual block
...
int32_t zkstake_nextheight()
{
    ...
    if ( (pindex= chainActive.Tip()) != 0 && (ht= pindex->nHeight) > 0 )
        return(ht+1);
    else return(zkstake_longestchain() + 1);
}
 ...
 * @return 1 if this chain's height >= zkstake_longestchain(), otherwise 0
 */
int32_t zkstake_isrealtime(int32_t *kmdheightp)
{
    AssertLockHeld(cs_main);

    zkstake_state *sp = zkstake_stateptrget( (char*)"KMD");
    if ( sp != nullptr )
        *kmdheightp = sp->CURRENT_HEIGHT;
    else 
        *kmdheightp = 0;
    CBlockIndex *pindex = chainActive.Tip();
    if ( pindex != nullptr && pindex->nHeight >= (int32_t)zkstake_longestchain() )
        return 1;
    return 0;
}
 ...
 */
bool zkstake_validate_interest(const CTransaction &tx,int32_t txheight,uint32_t cmptime)
{
    if ( ZKSTAKE_REWIND == 0 && chainName.isKMD() && (int64_t)tx.nLockTime >= LOCKTIME_THRESHOLD ) //1473793441 )
    {
        if ( txheight > 246748 ) // a long time ago
        {
            if ( txheight < 247205 ) // a long time ago
                cmptime -= 16000; // subtract about 4 1/2 hours
            if ( (int64_t)tx.nLockTime < cmptime-ZKSTAKE_MAXMEMPOOLTIME )
            {  
                // transaction has been in mempool for more than an hour
                if ( tx.nLockTime != 1477258935 )
                {
                    LogPrintf("zkstake_validate_interest.%d reject.%d [%d] locktime %u cmp2.%u\n",1,txheight,(int32_t)(tx.nLockTime - (cmptime-ZKSTAKE_MAXMEMPOOLTIME)),(uint32_t)tx.nLockTime,cmptime);
                }
                return false;
            }
        }
...
/*
 zkstake_checkPOW (fast) is called early in the process and should only refer to data immediately available. it is a filter to prevent bad blocks from going into the local DB. The more blocks we can filter out at this stage, the less junk in the local DB that will just get purged later on.

 zkstake_checkPOW (slow) is called right before connecting blocks so all prior blocks can be assumed to be there and all checks must pass
...
uint64_t zkstake_commission(const CBlock *pblock,int32_t height)
{
...
uint32_t zkstake_segid32(char *coinaddr)
{
    bits256 addrhash;
...
int8_t zkstake_segid(int32_t nocache,int32_t height)
{
    ...
    if ( height > 0 && (pindex= zkstake_chainactive(height)) != 0 )
    {
        if ( nocache == 0 && pindex->segid >= -1 )
            return(pindex->segid);
        if ( zkstake_blockload(block,pindex) == 0 )
        {
            newStakerActive = zkstake_newStakerActive(height, block.nTime);
            txn_count = block.vtx.size();
            if ( txn_count > 1 && block.vtx[txn_count-1].vin.size() == 1 && block.vtx[txn_count-1].vout.size() == 1+zkstake_hasOpRet(height,pindex->nTime) )
            {
                ...
                if ( ExtractDestination(block.vtx[txn_count-1].vout[0].scriptPubKey,voutaddress) )
                {
                    ...
                    {
                        segid = zkstake_segid32(voutaddr) & 0x3f;
                        //LogPrintf( "zkstake_segid: ht.%i --> %i\n",height,pindex->segid);
                    }
                } //else LogPrintf("zkstake_segid ht.%d couldnt extract voutaddress\n",height);
            }
        }
        // The new staker sets segid in zkstake_checkPOW, this persists after restart by being saved in the blockindex for blocks past the HF timestamp, to keep backwards compatibility.
...
void zkstake_segids(uint8_t *hashbuf,int32_t height,int32_t n)
{
    ...
    else
    {
        ...
        for (i=0; i<n; i++)
        {
            hashbuf[i] = (uint8_t)zkstake_segid(0,height+i);
...
uint32_t zkstake_stakehash(uint256 *hashp,char *address,uint8_t *hashbuf,uint256 txid,int32_t vout)
{
    bits256 addrhash;
...
arith_uint256 zkstake_PoWtarget(int32_t *percPoSp,arith_uint256 target,int32_t height,int32_t goalperc,int32_t newStakerActive)
{
    ...
    *percPoSp = percPoS = 0;
    ...
    for (i=n=m=0; i<100; i++)
    {
        ...
        if ( ht <= 1 )
            continue;
        if ( (pindex= zkstake_chainactive(ht)) != 0 )
        {
            if ( zkstake_segid(0,ht) >= 0 )
            {
                n++;
                percPoS++;
...
uint32_t zkstake_stake(int32_t validateflag,arith_uint256 bnTarget,int32_t nHeight,uint256 txid,int32_t vout,uint32_t blocktime,uint32_t prevtime,char *destaddr,int32_t PoSperc)
{
    ...
    txtime = zkstake_txtime2(&value,txid,vout,address);
    if ( validateflag == 0 )
    {
    ...
    {
        //LogPrintf("zkstake_stake null %.8f %u %u %u\n",dstr(value),txtime,blocktime,prevtime);
        return(0);
    }
    if ( value < SATOSHIDEN )
        return(0);
    ...
    if ( (minage= nHeight*3) > 6000 ) // about 100 blocks
        minage = 6000;
    zkstake_segids(hashbuf,nHeight-101,100);
    segid32 = zkstake_stakehash(&hash,address,hashbuf,txid,vout);
...
int32_t zkstake_is_PoSblock(int32_t slowflag,int32_t height,CBlock *pblock,arith_uint256 bnTarget,arith_uint256 bhash)
{
    ...
    int32_t newStakerActive = zkstake_newStakerActive(height, pblock->nTime);
    // Get PoSperc and POW Target. slowflag only here, calling it when blocks out of order causes problems.
    if ( slowflag != 0 )
    {
        POWTarget = zkstake_PoWtarget(&PoSperc,bnTarget,height,ASSETCHAINS_STAKED,newStakerActive);
    }
    else 
    {
        // checks opret merkle root and existence of staking tx.
        return(zkstake_isPoS(pblock,height,0));
    }
    txn_count = pblock->vtx.size();
    ...
    if ( txn_count > 1 && pblock->vtx[txn_count-1].vin.size() == 1 && pblock->vtx[txn_count-1].vout.size() == 1+zkstake_hasOpRet(height,pblock->nTime) )
    {
        ...
        {
            if ( zkstake_isPoS(pblock,height,0) != 0 ) 
            {
                // checks utxo is eligible to stake this block
                eligible = zkstake_stake(1,bnTarget,height,txid,vout,pblock->nTime,prevtime+ASSETCHAINS_STAKED_BLOCK_FUTURE_HALF,(char *)"",PoSperc); 
            }
            if ( eligible == 0 || eligible > pblock->nTime )
            {
                if ( 0 && ASSETCHAINS_STAKED < 100 ) 
                    LogPrintf("zkstake_is_PoSblock PoS failure ht.%d eligible.%u vs blocktime.%u, lag.%d -> check to see if it is PoW block\n",height,eligible,(uint32_t)pblock->nTime,(int32_t)(eligible - pblock->nTime));
            }
            else 
            {
                isPoS = 1;
...
uint64_t zkstake_notarypayamount(int32_t nHeight, int64_t notarycount)
{
    ...
    if ( notarycount == 0 )
    {
        LogPrintf( "zkstake_notarypayamount failed num notaries is 0!\n");
        return(0);
    }
...
int32_t zkstake_getnotarizedheight(uint32_t timestamp,int32_t height, uint8_t *script, int32_t len)
{
    ...
    if ( len >= sizeof(uint32_t) && len <= sizeof(scriptbuf) )
    {
        ...
        if ( zkstake_voutupdate(true,&isratification,0,scriptbuf,len,height,uint256(),1,1,&voutmask,&specialtx,&notarizedheight,0,1,0,timestamp) != -2 )
        {
...
uint64_t zkstake_notarypay(CMutableTransaction &txNew, std::vector<int8_t> &NotarisationNotaries, uint32_t timestamp, int32_t height, uint8_t *script, int32_t len)
{
    // fetch notary pubkey array.
    uint64_t total = 0, AmountToPay = 0;
    int8_t numSN = 0; uint8_t notarypubkeys[64][33] = {0};
    numSN = zkstake_notaries(notarypubkeys, height, timestamp);
    ...
    int32_t notarizedheight = zkstake_getnotarizedheight(timestamp, height, script, len);
    ...
    AmountToPay = zkstake_notarypayamount(height,NotarisationNotaries.size());
    if ( AmountToPay == 0 )
        return(0);
...
uint64_t zkstake_checknotarypay(CBlock *pblock,int32_t height)
{
    ...
    if ( pblock->vtx[1].vout.size() == 2 && pblock->vtx[1].vout[1].nValue == 0 )
    {
        ...
        if ( script[0] == OP_RETURN )
        {
            ...
            totalsats = zkstake_notarypay(txNew, NotarisationNotaries, pblock->nTime, height, script, scriptlen);
        } 
        else 
        {
...
bool zkstake_appendACscriptpub()
{
    ...
    if ( ASSETCHAINS_SCRIPTPUB[ASSETCHAINS_SCRIPTPUB.back()] == 49 && ASSETCHAINS_SCRIPTPUB[ASSETCHAINS_SCRIPTPUB.back()-1] == 51 )
    {
        ...
        if ( myGetTransaction(ZKSTAKE_EARLYTXID,tx,blockhash) && mapBlockIndex[blockhash]->nHeight < ZKSTAKE_EARLYTXID_HEIGHT )
        {
        ...
        LogPrintf( "could not get ZKSTAKE_EARLYTXID.%s OP_RETURN data. Restart with correct txid!\n", ZKSTAKE_EARLYTXID.GetHex().c_str());
        StartShutdown();
    }
    return false;
}

void GetZKstakeEarlytxidScriptPub()
{
    AssertLockHeld(cs_main);
    if ( ZKSTAKE_EARLYTXID == zeroid )
    {
        LogPrintf( "Restart deamon with -earlytxid.\n");
        StartShutdown();
        return;
    }
    if ( chainActive.Height() < ZKSTAKE_EARLYTXID_HEIGHT )
    {
        LogPrintf("Cannot fetch -earlytxid before block %d.\n",ZKSTAKE_EARLYTXID_HEIGHT);
        StartShutdown();
        return;
    }
    ...
    if ( myGetTransaction(ZKSTAKE_EARLYTXID,tx,blockhash) && mapBlockIndex[blockhash]->nHeight < ZKSTAKE_EARLYTXID_HEIGHT )
    {
        ...
        if ( i < tx.vout.size() )
        {
            ZKSTAKE_EARLYTXID_SCRIPTPUB = CScript(tx.vout[i].scriptPubKey.begin()+3, tx.vout[i].scriptPubKey.end());
            LogPrintf( "ZKSTAKE_EARLYTXID_SCRIPTPUB.%s\n", HexStr(ZKSTAKE_EARLYTXID_SCRIPTPUB.begin(),ZKSTAKE_EARLYTXID_SCRIPTPUB.end()).c_str());
            return;
        }
    }
...
int64_t zkstake_checkcommission(CBlock *pblock,int32_t height)
{
    ...
    if ( ASSETCHAINS_COMMISSION != 0 || ASSETCHAINS_FOUNDERS_REWARD != 0 )
    {
        ...
        else if ( checktoshis != 0 )
        {
            ...
            {
                static bool didinit = false;
                if ( !didinit && height > ZKSTAKE_EARLYTXID_HEIGHT && ZKSTAKE_EARLYTXID != zeroid && zkstake_appendACscriptpub() )
                {
...
bool ZKSTAKE_TEST_ASSETCHAIN_SKIP_POW = 0;

int32_t zkstake_checkPOW(int64_t stakeTxValue, int32_t slowflag,CBlock *pblock,int32_t height)
{
    ...
    if ( ZKSTAKE_TEST_ASSETCHAIN_SKIP_POW == 0 && Params().NetworkIDString() == "regtest" )
        ZKSTAKE_TEST_ASSETCHAIN_SKIP_POW = 1;
    if ( !CheckEquihashSolution(pblock, Params()) )
    {
        LogPrintf("zkstake_checkPOW slowflag.%d ht.%d CheckEquihashSolution failed\n",slowflag,height);
        return(-1);
    }
    hash = pblock->GetHash();
    bnTarget.SetCompact(pblock->nBits,&fNegative,&fOverflow);
    bhash = UintToArith256(hash);
    possible = zkstake_block2pubkey33(pubkey33,pblock);
    if ( height == 0 )
    {
        if ( slowflag != 0 )
        {
    ...
    if ( ( !chainName.isKMD() || height > 792000) && bhash > bnTarget )
    {
        ...
        {
            if ( (n= zkstake_notaries(pubkeys,height,pblock->nTime)) > 0 )
            {
                for (i=0; i<n; i++)
        ...
        {
            if ( ZKSTAKE_TEST_ASSETCHAIN_SKIP_POW )
                return(0);
            if ( ASSETCHAINS_STAKED == 0 ) // zkstake_is_PoSblock will check bnTarget for staked chains
                return(-1);
        }
    }
    ...
    if ( ASSETCHAINS_STAKED != 0 && height >= 2 ) // must PoS or have at least 16x better PoW
    {
        ...
        pindex = it != mapBlockIndex.end() ? it->second : NULL;
        int32_t newStakerActive = zkstake_newStakerActive(height, pblock->nTime);
        if ( (is_PoSblock= zkstake_is_PoSblock(slowflag,height,pblock,bnTarget,bhash)) == 0 )
        {
            ...
            {
                bnTarget = zkstake_PoWtarget(&PoSperc,bnTarget,height,ASSETCHAINS_STAKED,newStakerActive);
                if ( bhash > bnTarget ) 
                {
                    for (i=31; i>=16; i--)
    ...
    if ( failed == 0 && ASSETCHAINS_COMMISSION != 0 ) 
    {
        ...
        {
            if ( zkstake_checkcommission(pblock,height) < 0 )
                return(-1);
        }
    }
    ...
    if ( failed == 0 && ASSETCHAINS_NOTARY_PAY[0] != 0 && pblock->vtx.size() > 1 )
    {
        ...
        if ( slowflag == 0 && pblock->vtx[0].vout.size() > 1 )
        {
            ...
            if ( !zkstake_is_notarytx(pblock->vtx[1]) )
            {
            ...
            // Check min sigs.
            int8_t numSN = 0; uint8_t notarypubkeys[64][33] = {0};
            numSN = zkstake_notaries(notarypubkeys, height, pblock->nTime);
...
//LogPrintf("zkstake_checkPOW possible.%d slowflag.%d ht.%d notaryid.%d failed.%d\n",possible,slowflag,height,notaryid,failed);
    if ( failed != 0 && possible == 0 && notaryid < 0 )
        return(-1);
    else return(0);
}

int32_t zkstake_acpublic(uint32_t tiptime)
{
    ...
    if ( acpublic == 0 )
    {
        ...
        if ( (chainName.isKMD() || chainName.isSymbol("ZEX")) && tiptime >= ZKSTAKE_SAPLING_DEADLINE )
            acpublic = 1;
    }
    return(acpublic);
}

int64_t zkstake_newcoins(int64_t *zfundsp,int64_t *sproutfundsp,int32_t nHeight,CBlock *pblock)
{
...
int64_t zkstake_coinsupply(int64_t *zfundsp,int64_t *sproutfundsp,int32_t height)
{
    CBlockIndex *pindex; CBlock block; int64_t zfunds=0,sproutfunds=0,supply = 0;
    //LogPrintf("coinsupply %d\n",height);
    *zfundsp = *sproutfundsp = 0;
    if ( (pindex= zkstake_chainactive(height)) != 0 )
    {
        while ( pindex != 0 && pindex->nHeight > 0 )
        {
            ...
            {
                if ( zkstake_blockload(block,pindex) == 0 )
                    pindex->newcoins = zkstake_newcoins(&pindex->zfunds,&pindex->sproutfunds,pindex->nHeight,&block);
                else
                {
...
void zkstake_addutxo(std::vector<zkstake_staking> &array,uint32_t txtime,uint64_t nValue,uint256 txid,int32_t vout,char *address,uint8_t *hashbuf,CScript pk)
{
    uint256 hash; uint32_t segid32; zkstake_staking kp;
    segid32 = zkstake_stakehash(&hash,address,hashbuf,txid,vout);
...
int32_t zkstake_staked(CMutableTransaction &txNew,uint32_t nBits,uint32_t *blocktimep,uint32_t *txtimep,uint256 *utxotxidp,int32_t *utxovoutp,uint64_t *utxovaluep,uint8_t *utxosig, uint256 merkleroot)
{
    // use thread_local to prevent crash in case of accidental thread overlapping
    thread_local std::vector<zkstake_staking> array; 
    ...
    *utxovaluep = 0;
    ...
    //tmpTarget = zkstake_PoWtarget(&PoSperc,bnTarget,nHeight,ASSETCHAINS_STAKED);
    ...
    if ( array.size() != 0 )
    {
        ...
        if ( ReadBlockFromDisk(block, pblockindex, 1) && zkstake_isPoS(&block, nHeight, &addressout) != 0 && IsMine(*pwalletMain,addressout) != 0 )
        {
              resetstaker = true;
    ...
    if ( resetstaker || array.size() == 0 || time(NULL) > lasttime+600 )
    {
        ...
        BOOST_FOREACH(const COutput& out, vecOutputs)
        {
            ...
            {
                //LogPrintf("zkstake_staked invalid depth %d\n",(int32_t)out.nDepth);
                continue;
            }
            ...
            if ( ExtractDestination(pk,address) != 0 )
            {
                if ( IsMine(*pwalletMain,address) == 0 )
                    continue;
                if ( myGetTransaction(out.tx->GetHash(),tx,hashBlock) != 0 && (pindex= zkstake_getblockindex(hashBlock)) != 0 )
                {
                    zkstake_addutxo(array,(uint32_t)pindex->nTime,(uint64_t)nValue,out.tx->GetHash(),out.i,(char *)CBitcoinAddress(address).ToString().c_str(),hashbuf,(CScript)pk);
    ...
    for (i=winners=0; i<array.size(); i++)
    {
        ...
        zkstake_staking &kp = array[i];
        eligible = zkstake_stake(0,bnTarget,nHeight,kp.txid,kp.vout,0,(uint32_t)tipindex->nTime+ASSETCHAINS_STAKED_BLOCK_FUTURE_HALF,kp.address,PoSperc);
        if ( eligible > 0 )
        {
            besttime = 0;
            if ( eligible == zkstake_stake(1,bnTarget,nHeight,kp.txid,kp.vout,eligible,(uint32_t)tipindex->nTime+ASSETCHAINS_STAKED_BLOCK_FUTURE_HALF,kp.address,PoSperc) )
            {
    ...
    if ( earliest != 0 )
    {
        ...
        if ( (newStakerActive= zkstake_newStakerActive(nHeight,earliest)) != 0 )
        {
