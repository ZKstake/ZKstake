...
uint16_t BITCOIND_RPCPORT = 17838;
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("ZKstake RPC client version") + " " + FormatFullVersion() + "\n" + PrivacyInfo();
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  zkstake-cli [options] <command> [params]  " + _("Send command to ZKstake") + "\n" +
                  "  zkstake-cli [options] help                " + _("List commands") + "\n" +
                  "  zkstake-cli [options] help <command>      " + _("Get help for a command") + "\n";
