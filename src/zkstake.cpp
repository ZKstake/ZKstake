...
//int32_t ZKSTAKE_EXTERNAL_NOTARIES = 0; //todo remove
...
{
    char symbol[ZKSTAKE_ASSETCHAIN_MAXLEN],dest[ZKSTAKE_ASSETCHAIN_MAXLEN]; struct zkstake_state *sp;
    if ( (sp= zkstake_stateptr(symbol,dest)) != 0 )
    ...
    try
    {
        if ( (func= fgetc(fp)) != EOF )
        {
            ...
            if ( fread(&ht,1,sizeof(ht),fp) != sizeof(ht) )
                throw zkstake::parse_error("Unable to read height from file");
            if ( func == 'P' )
            {
                zkstake::event_pubkeys pk(fp, ht);
                if ( (ZKSTAKE_EXTERNAL_NOTARIES && matched ) || (strcmp(symbol,"KMD") == 0 && !ZKSTAKE_EXTERNAL_NOTARIES) )
                {
                    zkstake_eventadd_pubkeys(sp, symbol, ht, pk);
                }
            }
            else if ( func == 'N' || func == 'M' )
            {
                zkstake::event_notarized evt(fp, ht, dest, func == 'M');
                zkstake_eventadd_notarized(sp, symbol, ht, evt);
            }
            else if ( func == 'U' ) // deprecated
            {
                zkstake::event_u evt(fp, ht);
            }
            else if ( func == 'K' || func == 'T')
            {
                zkstake::event_kmdheight evt(fp, ht, func == 'T');
                zkstake_eventadd_kmdheight(sp, symbol, ht, evt);
            }
            else if ( func == 'R' )
            {
                zkstake::event_opreturn evt(fp, ht);
                // check for oversized opret
                if ( evt.opret.size() < 16384*4 )
                    zkstake_eventadd_opreturn(sp, symbol, ht, evt);
            }
            else if ( func == 'D' )
            {
            ...
            {
                zkstake::event_pricefeed evt(fp, ht);
                zkstake_eventadd_pricefeed(sp, symbol, ht, evt);
            }
            else if ( func == 'B' ) {
                // can be written but not processed on read
            }
            else {
                throw zkstake::parse_error("Unable to parse state file: unknown event");
            }
        } // retrieved the func
    }
    catch(const zkstake::parse_error& pe)
    {
        LogPrintf("Error occurred in parsestatefile: %s\n", pe.what());
        LogPrintf("%s file is invalid. ZKstaked will be stopped. Please remove %s and %s.ind files and start the daemon\n", ZKSTAKE_STATE_FILENAME, ZKSTAKE_STATE_FILENAME, ZKSTAKE_STATE_FILENAME);
        uiInterface.ThreadSafeMessageBox(strprintf("Please remove %s and %s.ind files and restart", ZKSTAKE_STATE_FILENAME, ZKSTAKE_STATE_FILENAME), "", CClientUIInterface::MSG_ERROR);
        StartShutdown();            
        func = -1;
    }
    return func;
    ...
    try
    {
        ...
        if ( fpos < datalen )
        {
            ...
            if ( mem_read(ht, filedata, fpos, datalen) != sizeof(ht) )
                throw zkstake::parse_error("Unable to parse height from file data");
            if ( func == 'P' )
            {
                zkstake::event_pubkeys pk(filedata, fpos, datalen, ht);
                if ( (ZKSTAKE_EXTERNAL_NOTARIES && matched ) || (strcmp(symbol,"KMD") == 0 && !ZKSTAKE_EXTERNAL_NOTARIES) )
                {
                    zkstake_eventadd_pubkeys(sp, symbol, ht, pk);
                }
            ...
            {
                zkstake::event_notarized ntz(filedata, fpos, datalen, ht, dest, func == 'M');
                zkstake_eventadd_notarized(sp, symbol, ht, ntz);
            }
            else if ( func == 'U' ) // deprecated
            {
                zkstake::event_u u(filedata, fpos, datalen, ht);
            }
            else if ( func == 'K' || func == 'T' )
            {
                zkstake::event_kmdheight kmd_ht(filedata, fpos, datalen, ht, func == 'T');
                zkstake_eventadd_kmdheight(sp, symbol, ht, kmd_ht);
            }
            else if ( func == 'R' )
            {
                zkstake::event_opreturn opret(filedata, fpos, datalen, ht);
                zkstake_eventadd_opreturn(sp, symbol, ht, opret);
            }
            else if ( func == 'D' )
            {
            ...
            {
                zkstake::event_pricefeed pf(filedata, fpos, datalen, ht);
                zkstake_eventadd_pricefeed(sp, symbol, ht, pf);
            }
            else if ( func == 'B' ) {
                // can be written but not processed on read
            }
            else {
                throw zkstake::parse_error("Unable to parse file data: unknown event");
            }
            *fposp = fpos;
        }
    }
...
void zkstake_stateupdate(int32_t height,uint8_t notarypubs[][33],uint8_t numnotaries,
        ...
        uint8_t *opretbuf,uint16_t opretlen,uint16_t vout,uint256 MoM,int32_t MoMdepth)
{
    ...
    static uint256 zero;
    struct zkstake_state *sp; 
    char fname[MAX_STATEFNAME+1],symbol[ZKSTAKE_ASSETCHAIN_MAXLEN],dest[ZKSTAKE_ASSETCHAIN_MAXLEN]; 
    int32_t ht,func;
    uint8_t num,pubkeys[64][33];

    if ( didinit == 0 )
    {
        portable_mutex_init(&ZKSTAKE_CC_mutex);
        didinit = 1;
    }
    if ( (sp= zkstake_stateptr(symbol,dest)) == 0 )
    {
        ZKSTAKE_INITDONE = (uint32_t)time(NULL);
        LogPrintf("[%s] no zkstake_stateptr\n",chainName.symbol().c_str());
        return;
    }
    if ( fp == 0 )
    {
        zkstake_statefname(fname, chainName.symbol().c_str(), ZKSTAKE_STATE_FILENAME);
        if ( (fp= fopen(fname,"rb+")) != nullptr )
        {
            if ( zkstake_faststateinit(sp, fname, symbol, dest) )
                fseek(fp,0,SEEK_END);
            else
            {
                // unable to use faststateinit, so try again only slower
                LogPrintf("zkstake_faststateinit retval.-1\n");
                while (!ShutdownRequested() && zkstake_parsestatefile(sp,fp,symbol,dest) >= 0)
                    ;
            }
            LogPrintf("zkstake read last notarised height %d from %s\n", sp->LastNotarizedHeight(), ZKSTAKE_STATE_FILENAME);
        } 
        else 
    ...
    if ( fp != 0 ) // write out funcid, height, other fields, call side effect function
    {
        if ( KMDheight != 0 )
        {
            zkstake::event_kmdheight kmd_ht(height);
            kmd_ht.kheight = KMDheight;
            kmd_ht.timestamp = KMDtimestamp;
            write_event(kmd_ht, fp);
            zkstake_eventadd_kmdheight(sp,symbol,height,kmd_ht);
        }
        else if ( opretbuf != 0 && opretlen > 0 )
        {
            zkstake::event_opreturn evt(height);
            evt.txid = txhash;
            evt.vout = vout;
            ...
            write_event(evt, fp);
            zkstake_eventadd_opreturn(sp,symbol,height,evt);
        }
        else if ( notarypubs != 0 && numnotaries > 0 )
        {
            zkstake::event_pubkeys pk(height);
            pk.num = numnotaries;
            memcpy(pk.pubkeys, notarypubs, 33 * 64);
            write_event(pk, fp);
            zkstake_eventadd_pubkeys(sp,symbol,height,pk);
        }
        /* TODO: why is this removed in jmj_event_fix3?
        else if ( voutmask != 0 && numvouts > 0 )
        {
            zkstake::event_u evt(height);
            evt.n = numvouts;
            evt.nid = notaryid;
        ...
        else if ( pvals != 0 && numpvals > 0 )
        {
            ...
            if ( nonz >= 32 )
            {
                zkstake::event_pricefeed evt(height);
                evt.num = numpvals;
                for( uint8_t i = 0; i < evt.num; ++i)
                    evt.prices[i] = pvals[i];
                write_event(evt, fp);
                zkstake_eventadd_pricefeed(sp,symbol,height,evt);
            }
        }
        else if ( height != 0 )
        {
            if ( sp != nullptr )
            {
                zkstake::event_notarized evt(height, dest);
                ...
                write_event(evt, fp);
                zkstake_eventadd_notarized(sp,symbol,height,evt);
            }
        }
        fflush(fp);
    }
...
int32_t zkstake_validate_chain(uint256 srchash,int32_t notarized_height)
{
    static int32_t last_rewind; int32_t rewindtarget; CBlockIndex *pindex; struct zkstake_state *sp; char symbol[ZKSTAKE_ASSETCHAIN_MAXLEN],dest[ZKSTAKE_ASSETCHAIN_MAXLEN];
    if ( (sp= zkstake_stateptr(symbol,dest)) == 0 )
        return(0);
    if ( IsInitialBlockDownload() == 0 && ((pindex= zkstake_getblockindex(srchash)) == 0 || pindex->nHeight != notarized_height) )
    {
        ...
        if ( rewindtarget != 0 && rewindtarget > ZKSTAKE_REWIND && rewindtarget > last_rewind )
        {
            if ( last_rewind != 0 )
            {
...
int32_t zkstake_voutupdate(bool fJustCheck,int32_t *isratificationp,int32_t notaryid,uint8_t *scriptbuf,
        ...
        uint64_t signedmask,uint32_t timestamp)
{
    ...
    int32_t opretlen,nid,offset,k,MoMdepth,matched,len = 0; uint256 MoM,srchash,desttxid; uint8_t crypto777[33]; struct zkstake_state *sp; char symbol[ZKSTAKE_ASSETCHAIN_MAXLEN],dest[ZKSTAKE_ASSETCHAIN_MAXLEN];
    if ( (sp= zkstake_stateptr(symbol,dest)) == 0 )
        return(-1);
    if ( scriptlen == 35 && scriptbuf[0] == 33 && scriptbuf[34] == 0xac )
    {
        if ( i == 0 && j == 0 && memcmp(NOTARY_PUBKEY33,scriptbuf+1,33) == 0 && IS_ZKSTAKE_NOTARY )
        {
            LogPrintf("%s ZKSTAKE_LASTMINED.%d -> %d\n",chainName.symbol().c_str(),ZKSTAKE_LASTMINED,height);
            prevZKSTAKE_LASTMINED = ZKSTAKE_LASTMINED;
            ZKSTAKE_LASTMINED = height;
        }
        ...
        else if ( zkstake_chosennotary(&nid,height,scriptbuf + 1,timestamp) >= 0 )
        {
    ...
    {
        struct zkstake_ccdata ccdata; struct zkstake_ccdataMoMoM MoMoMdata;
        ...
        else
        {
            if ( scriptbuf[len] == 'K' )
            {
                ...
                zkstake_stateupdate(height,0,0,0,txhash,0,0,0,0,value,&scriptbuf[len],opretlen,j,zero,0);
                return(-1);
            }
        ...
        if ( j == 1 && opretlen >= len+offset-opoffset )
        {
            ...
            if ( matched == 0 && signedmask != 0 && bitweight(signedmask) >= ZKSTAKE_MINRATIFY )
                notarized = 1;
            ...
            if ( matched != 0 )
                validated = zkstake_validate_chain(srchash,*notarizedheightp);
            else validated = 1;
            ...
            if ( notarized != 0 && validated != 0 )
            {
                    ...
                    if ( len+sizeof(ccdata.CCid)-opoffset <= opretlen )
                    {
                        ...
                        if ( !chainName.isKMD() )
                        {
                            ...
                            if ( len+48-opoffset <= opretlen && chainName.isSymbol(ccdata.symbol) )
                            {
                                ...
                                {
                                    MoMoMdata.pairs = (struct zkstake_ccdatapair *)calloc(MoMoMdata.numpairs,sizeof(*MoMoMdata.pairs));
                ...
                if ( matched != 0 && *notarizedheightp > sp->LastNotarizedHeight() && *notarizedheightp < height )
                {
                    ...
                    zkstake_stateupdate(height,0,0,0,zero,0,0,0,0,0,0,0,0,sp->LastNotarizedMoM(),sp->LastNotarizedMoMDepth());
                    ...
                    if ( chainName.isKMD() )
                    {
                        if ( signedfp == 0 )
                        {
                            ...
                            zkstake_statefname(fname,chainName.symbol().c_str(),(char *)"signedmasks");
                        ...
                        if ( opretlen > len && scriptbuf[len] == 'A' )
                        {
                            ...
                            zkstake_stateupdate(height,0,0,0,txhash,0,0,0,0,value,&scriptbuf[len],opretlen-len+4+3+(scriptbuf[1] == 0x4d),j,zero,0);
                        }
                    }
                }
...
// with more than ZKSTAKE_MINRATIFY pay2pubkey outputs -> ratify
...
int32_t zkstake_notarycmp(uint8_t *scriptPubKey,int32_t scriptlen,uint8_t pubkeys[64][33],int32_t numnotaries,uint8_t rmd160[20])
{
    int32_t i;
...
*/
/* 
    JMJ: Moved hwmheight out of zkstake_connectblock to allow testing. 
    Adjusting this should only be done by zkstake_connectblock or a unit test
*/
...
int32_t zkstake_connectblock(bool fJustCheck, CBlockIndex *pindex,CBlock& block)
{
    ...
    uint64_t signedmask,voutmask; char symbol[ZKSTAKE_ASSETCHAIN_MAXLEN],dest[ZKSTAKE_ASSETCHAIN_MAXLEN]; struct zkstake_state *sp;
    ...
    if ( pindex == 0 )
    {
        LogPrintf("zkstake_connectblock null pindex\n");
        return(0);
    }
    memset(&zero,0,sizeof(zero));
    zkstake_init(pindex->nHeight);
    ZKSTAKE_INITDONE = (uint32_t)time(NULL);
    if ( (sp= zkstake_stateptr(symbol,dest)) == 0 )
    {
        LogPrintf("unexpected null zkstakestateptr.[%s]\n",chainName.symbol().c_str());
        return(0);
    }
    ...
    numnotaries = zkstake_notaries(pubkeys,pindex->nHeight,pindex->GetBlockTime());
    ...
    else
    {
        if ( pindex->nHeight != hwmheight )
        {
            ...
            zkstake_purge_ccdata((int32_t)pindex->nHeight);
            hwmheight = pindex->nHeight;
        }
        if (!fJustCheck)
        {
            zkstake_event_rewind(sp,symbol,pindex->nHeight);
            zkstake_stateupdate(pindex->nHeight,0,0,0,zero,0,0,-pindex->nHeight,pindex->nTime,0,0,0,0,zero,0);
        }
    }
    ...
    if ( pindex != 0 )
    {
        ...
        for (i=0; i<txn_count; i++)
        {
            ...
            for (j=0; j<numvins; j++)
            {
                ...
                {
                    if ( (k= zkstake_notarycmp(scriptPubKey,scriptlen,pubkeys,numnotaries,rmd160)) >= 0 )
                        signedmask |= (1LL << k);
            ...
            if ( ((height < 90000 || (signedmask & 1) != 0) && numvalid >= ZKSTAKE_MINRATIFY) 
                    || (numvalid >= ZKSTAKE_MINRATIFY && !chainName.isKMD()) 
            ...
            for (j=0; j<numvouts; j++)
            {
                ...
                if ( len >= sizeof(uint32_t) && len <= sizeof(scriptbuf) )
                {
                    ...
                    notaryid = zkstake_voutupdate(fJustCheck,&isratification,notaryid,scriptbuf,len,height,txhash,i,j,&voutmask,&specialtx,&notarizedheight,(uint64_t)block.vtx[i].vout[j].nValue,notarized,signedmask,(uint32_t)chainActive.Tip()->GetBlockTime());
            ...
            if ( !fJustCheck && (notarized != 0 && (notarizedheight != 0 || specialtx != 0)) )
            {
                ...
                if ( specialtx != 0 && isratification != 0 && numvouts > 2 )
                {
                    ...
                    {
                        if ( ((signedmask & 1) != 0 && numvalid >= ZKSTAKE_MINRATIFY) || bitweight(signedmask) > (numnotaries/3) )
                        {
                            memset(&txhash,0,sizeof(txhash));
                            zkstake_stateupdate(height,pubkeys,numvalid,0,txhash,0,0,0,0,0,0,0,0,zero,0);
                            LogPrintf("RATIFIED! >>>>>>>>>> new notaries.%d newheight.%d from height.%d\n",numvalid,(((height+ZKSTAKE_ELECTION_GAP/2)/ZKSTAKE_ELECTION_GAP)+1)*ZKSTAKE_ELECTION_GAP,height);
                        } else LogPrintf("signedmask.%llx numvalid.%d wt.%d numnotaries.%d\n",(long long)signedmask,numvalid,bitweight(signedmask),numnotaries);
                    }
                }
            }
        }
        if ( !fJustCheck && IS_ZKSTAKE_NOTARY && chainName.isKMD() )
            LogPrintf("%s ht.%d\n",chainName.ToString().c_str(),height);
        if ( !fJustCheck && pindex->nHeight == hwmheight )
            zkstake_stateupdate(height,0,0,0,zero,0,0,height,(uint32_t)pindex->nTime,0,0,0,0,zero,0);
    } 
    else 
        { LogPrintf("zkstake_connectblock: unexpected null pindex\n"); return(0); }
    if (fJustCheck)
    {
        ...
        LogPrintf("zkstake_connectblock: unxexpected behaviour when fJustCheck == true, report blackjok3rtt plz ! \n");
