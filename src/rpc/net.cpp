...
int32_t zkstake_longestchain()
{
    ...
    if ( depth == 0 )
    {
        ...
        BOOST_FOREACH(const CNodeStats& stats, vstats)
        {
            //fprintf(stderr,"zkstake_longestchain iter.%d\n",n);
            CNodeStateStats statestats;
            bool fStateStats = GetNodeStateStats(stats.nodeid,statestats);
        ...
        if ( num > (n >> 1) )
        {
            if ( 0 && height != ZKSTAKE_LONGESTCHAIN )
                LogPrintf("set %s ZKSTAKE_LONGESTCHAIN <- %d\n",chainName.symbol().c_str(),height);
            ZKSTAKE_LONGESTCHAIN = height;
            return(height);
        }
        ZKSTAKE_LONGESTCHAIN = 0;
    }
    return(ZKSTAKE_LONGESTCHAIN);
}
...
UniValue getaddednodeinfo(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "    \"addresses\" : [\n"
            "       {\n"
            "         \"address\" : \"192.168.0.201:8233\",  (string) The ZKstake server host and port\n"
