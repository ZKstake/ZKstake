...
UniValue stop(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
   ...
   // Accept the deprecated and ignored 'detach' boolean argument
    ...
    StartShutdown();
    sprintf(buf,"%s server stopping", !chainName.isKMD() ? chainName.symbol().c_str() : "ZKstake");
    return buf;
}
...
{
    if ( chainName.isKMD() ) {
        return "> zkstake-cli " + methodname + " " + args + "\n";
    } else {
        return "> zkstake-cli -ac_name=" + strprintf("%s", chainName.symbol().c_str()) + " " + methodname + " " + args + "\n";
    }
}
...
{
    string daemon = "zkstaked";
    string ticker = chainName.isKMD() ? "zkstake" : chainName.symbol();
