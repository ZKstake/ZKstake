...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("confirmations", zkstake_dpowconfs(blockindex->nHeight,confirmations)));
    ...
    result.push_back(Pair("segid", (int)zkstake_segid(0,blockindex->nHeight)));
...
UniValue getlastsegidstakes(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    {
        int8_t segid = zkstake_segid(0,i);
...
/*uint256 _zkstake_getblockhash(int32_t nHeight)
{
    uint256 hash;
    LOCK(cs_main);
    if ( nHeight >= 0 && nHeight <= chainActive.Height() )
...
UniValue kvsearch(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( (keylen= (int32_t)strlen(params[0].get_str().c_str())) > 0 )
    {
        ...
        if ( keylen < sizeof(key) )
        {
            ...
            if ( (valuesize= zkstake_kvsearch(&refpubkey,chainActive.Tip()->nHeight,&flags,&height,value,key,keylen)) >= 0 )
            {
                ...
                duration = ((flags >> 2) + 1) * ZKSTAKE_KVDURATION;
...
UniValue minerids(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( (n= zkstake_minerids(minerids,height,(int32_t)(sizeof(minerids)/sizeof(*minerids)))) > 0 )
    {
        memset(tally,0,sizeof(tally));
        numnotaries = zkstake_notaries(pubkeys,height,timestamp);
        if ( numnotaries > 0 )
        {
            for (i=0; i<n; i++)
            {
...
UniValue notaries(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( (n= zkstake_notaries(pubkeys,height,timestamp)) > 0 )
    {
        for (i=0; i<n; i++)
        {
            UniValue item(UniValue::VOBJ);
...
UniValue gettxout(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of ZKstake addresses\n"
            "        \"zkstakeaddress\"        (string) ZKstake address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
    ...
    if ((unsigned int)coins.nHeight == MEMPOOL_HEIGHT) {
        ...
        ret.push_back(Pair("confirmations", zkstake_dpowconfs(coins.nHeight,pindex->nHeight - coins.nHeight + 1)));
    ...
    if ( (interest= zkstake_accrued_interest(&txheight,&locktime,hash,n,coins.nHeight,coins.vout[n].nValue,(int32_t)pindex->nHeight)) != 0 )
