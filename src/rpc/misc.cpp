...
extern int32_t ZKSTAKE_LASTMINED,JUMBLR_PAUSE,ZKSTAKE_LONGESTCHAIN,STAKED_NOTARY_ID,STAKED_ERA,ZKSTAKE_INSYNC;
...
UniValue getiguanajson(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    // get the min sigs .. this always rounds UP so min sigs in iguana is +1 min sigs in zkstaked, due to some rounding error.
    int minsigs;
...
UniValue getinfo(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total ZKstake balance of the wallet\n"
...
//#endif
    ...
    obj.push_back(Pair("KMDversion", ZKSTAKE_VERSION));
    obj.push_back(Pair("synced", ZKSTAKE_INSYNC!=0));
    ...
    if ( ZKSTAKE_NSPV_FULLNODE )
    {
        ...
        if (pwalletMain) {
            ...
            if ( chainName.isKMD() )
            {
                obj.push_back(Pair("interest",       ValueFromAmount(ZKSTAKE_INTERESTSUM)));
                obj.push_back(Pair("balance",       ValueFromAmount(ZKSTAKE_WALLETBALANCE))); //pwalletMain->GetBalance()
            }
            else
            {
        ...
        if ( (longestchain= ZKSTAKE_LONGESTCHAIN) != 0 && chainActive.Height() > longestchain )
    ...
    obj.push_back(Pair("errors",        GetWarnings("statusbar")));
     if ( NOTARY_PUBKEY33[0] != 0 ) {
        ...
        } else if( (notaryid= zkstake_whoami(pubkeystr,(int32_t)chainActive.Tip()->nHeight,zkstake_chainactive_timestamp())) >= 0 )  {
            obj.push_back(Pair("notaryid",        notaryid));
            if ( ZKSTAKE_LASTMINED != 0 )
                obj.push_back(Pair("lastmined", ZKSTAKE_LASTMINED));
        }
...
UniValue coinsupply(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (height >= 0 && height <= currentHeight) {
        if ( (supply= zkstake_coinsupply(&zfunds,&sproutfunds,height)) > 0 )
        {
            ...
            if ( ASSETCHAINS_BLOCKTIME > 0 )
            {
                ...
                {
                    supply1 = zkstake_coinsupply(&zf1,&sf1,height - blocks_per_year/12);
                    supply3 = zkstake_coinsupply(&zf3,&sf3,height - blocks_per_year/4);
                    supply12 = zkstake_coinsupply(&zf12,&sf12,height - blocks_per_year);
...
UniValue validateaddress(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"zkstakeaddress\"\n"
            "\nReturn information about the given ZKstake address.\n"
            "\nArguments:\n"
            "1. \"zkstakeaddress\"     (string, required) The ZKstake address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"zkstakeaddress\",   (string) The ZKstake address validated\n"
    ...
    if (isValid)
    {
        ...
        ret.push_back(Pair("segid", (int32_t)zkstake_segid32((char *)params[0].get_str().c_str()) & 0x3f));
#ifdef ENABLE_WALLET
...
UniValue createmultisig(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are ZKstake addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) ZKstake address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"zkstakeaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"zkstakeaddress\"    (string, required) The ZKstake address to use for the signature.\n"
...
UniValue getnotarypayinfo(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    notarycount = zkstake_notaries(notarypubkeys, height, chainActive[height]->GetBlockTime());
    NotaryPay = zkstake_notarypayamount(height, notarycount)*notarycount;
...
UniValue txnotarizedconfirmed(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    notarizedconfirmed=zkstake_txnotarizedconfirmed(txid);
    UniValue result(UniValue::VOBJ);
