...
bool AsyncRPCOperation_sendmany::main_impl() {
    ...
    if (isfromtaddr_) {
        ...
        if (isUsingBuilder_) {
            ...
            // for ZKstake, set lock time to accure interest, for other chains, set
            ...
            if (chainName.isKMD())
            {
                ...
                if ( !zkstake_hardfork_active((uint32_t)chainActive.Tip()->nTime) )
                    builder_.SetLockTime((uint32_t)time(NULL) - 60); // set lock time for ZKstake interest
                else
    ...
    while (!vpubNewProcessed) {
        ...
        while (numInputsNeeded++ < ZC_NUM_JS_INPUTS && zInputsDeque.size() > 0) {
            ...
            int wtxDepth = -1;
            {
                ...
                wtxHeight = zkstake_blockheight(wtx.hashBlock);
...
bool AsyncRPCOperation_sendmany::find_utxos(bool fAcceptCoinbase=false) {
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        if( mindepth_ > 1 ) {
            ...
            int dpowconfs  = zkstake_dpowconfs(nHeight, out.nDepth);
